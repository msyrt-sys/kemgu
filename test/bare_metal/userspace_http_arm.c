/*
 * USERSPACE HTTP GET testi (aarch64) — EL0 süreç SYSCALL ile UYGULAMA KATMANI.
 *
 * D-176 (userspace_net_arm.c) bir EL0 sürecin net_gonder(24)/net_al(25)
 * syscall'larıyla ham ethernet çerçevesi gönderip aldığını kanıtladı (ARP).
 * D-177 (userspace_dns_arm.c) aynı syscall'lar üstünde TAM L2-L7 DNS yığınını
 * EL0'a taşıdı. Bu test zirveye çıkar: bir EL0 (yetkisiz) süreç, virtio-net'e
 * DOĞRUDAN erişmeden, YALNIZ syscall ile TAM bir HTTP/1.1 istemcisi çalıştırır:
 *
 *   L2 (Ethernet) + L3 (IPv4 + checksum) + L4 (TCP handshake + pseudo-hdr csum)
 *   + L7 (HTTP GET isteği + yanıt durum satırı parse)
 *
 * http_get_arm.c (D-161) bu mantığı KERNEL'de (EL1) yapmıştı. Burada aynı mantık
 * EL0'a taşınır — ağ kernel-aracılıdır (sys2 24/25) ama TCP state makinesi +
 * HTTP protokolü tamamen userspace'te. Kanıt: bir userspace program, çekirdek-
 * aracılı ham-frame syscall'larıyla gerçek bir web sunucusundan sayfa çekti.
 *
 * Akış (EL0 launcher):
 ...
 *   5) HTTP: GET isteğini TCP DATA segmenti (PSH+ACK, flags=0x18) olarak yolla:
 *        "GET / HTTP/1.1\r\nHost: example.com\r\nConnection: close\r\n\r\n"
 *      seq = ISS+1 (handshake sonrası), ack = onların_iss+1.
 *   6) RX poll: ilk DATA segmentinde "HTTP/1." durum satırı ara → gelen data'ya
 *      ACK dön (seq/ack takibi) → durum satırını + kodu (200 veya 3xx) bas.
 *
 * KANIT (gerçek HTTP RX): "USERHTTP OK" (yanıtta HTTP/1.x durum satırı bulundu).
 * FALLBACK (host internet yoksa / SLIRP dış-TCP yanıt vermezse): EL0 GET isteğini
 * sys2(24) ile GÖNDERDİ → pcap'te "GET /" TX kanıtı → "USERHTTP SENT OK".
 *
 * GÜVENLİK (D-177 dersi): EL0 kernel .rodata (AP=00) DEREF EDEMEZ. Bu yüzden HTTP
 * request byte'ları kernel .rodata string literalinden OKUNMAZ — launcher onları
 * EL0 user yığınındaki (0x42000000 sayfası) bir tampona ELLE byte-byte yazar.
 * Tüm frame/rx/http tamponları user yığınında → sys2(24)/sys2(25) user-VA
 * guard'ından (D-150) geçer. sys(5) durum string'leri kernel tarafından okunur
 * (user-VA ∪ .rodata guard) → sorunsuz; ama EL0 kodu dereference eden HER şey
 * user sayfasında olmalı.
 */
#include <stdint.h>
...
/* Bir C literalini EL0 user tamponuna byte-byte kopyala (NUL dahil). Kaynak
 * literal EL0 kod sayfasında derlenir ama derleyici onu genelde .rodata'ya
 * koyar → EL0 dereference edemez. AMA: bu fonksiyon always_inline + parametre
 * literal DERLEME-ZAMANI sabiti olduğundan, çağıranın her sys(5) durum-string'i
 * kernel tarafından okunur (sorun yok). Kritik olan sys2(24)'e verilen frame
 * içindeki HTTP payload: onu aşağıda ELLE (indeksli atama) yazıyoruz, kopya YOK.
 * Bu yardımcı yalnız EL0'ın OKUYACAĞI kısa string'ler için (desen tamponu) var:
 * hedef user yığınında, kaynak inline sabitlerle doldurulur → .rodata deref YOK. */
...
/*
 * EL0 launcher: TAM HTTP istemcisi syscall üstünde.
...
static void launcher(void) {
    ...
    const uint32_t TCP_ISS = 0x4B454D47UL;   /* "KEMG" — pcap fallback marker + ISS */
    ...
    if (!resolve_ok) {
        sys(5, (unsigned long)(uintptr_t)"USERHTTP DNS YOK");
        sys(7, 0);
        for (;;) { }
    }

    sys(5, (unsigned long)(uintptr_t)"USERHTTP HEDEF IP=");
    ...
    /* --- HTTP GET isteği payload'ını EL0 user tamponuna ELLE byte-byte yaz ---
    ...
    int hl = 0;
    http[hl++]='G'; http[hl++]='E'; http[hl++]='T'; http[hl++]=' '; http[hl++]='/';
    ...
    /* --- 4) TCP SYN gönder: hedef_ip:80, seq=ISS ("KEMG") --- */
    ...
    if (!synack_ok) {
         ...
         * Handshake kurulamadı; yine de GET isteğini gönder ki pcap'te "GET /"
        ...
        (void)sys2(24, (unsigned long)(uintptr_t)frame, (unsigned long)toplam);
        sys(5, (unsigned long)(uintptr_t)"USERHTTP SENT OK");
        sys(7, 0);
        for (;;) { }
    }
    ...
    sys(5, (unsigned long)(uintptr_t)"USERHTTP TCP ESTABLISHED");
    sys(7, 0);

    /* --- 5) HTTP GET isteğini TCP DATA segmenti (PSH+ACK, flags=0x18) olarak yolla --- */
    toplam = tcp_segment_kur(frame, gw_mac, bizim_mac, bizim_ip, hedef_ip,
                             SRC_PORT, DST_PORT, bizim_seq, bizim_ack, 0x18,
                             http, http_uzun);   /* PSH+ACK + veri */
    (void)sys2(24, (unsigned long)(uintptr_t)frame, (unsigned long)toplam);
    ...
    if (!http_ok) {
        /* Yanıt gelmedi → pcap TX fallback: GET isteği gönderildi. */
        sys(5, (unsigned long)(uintptr_t)"USERHTTP SENT OK");
        sys(7, 0);
        for (;;) { }
    }
    ...
    sys(5, (unsigned long)(uintptr_t)"USERHTTP DURUM: ");
    ...
    if (kod_off + 2 < rx_uzun) {
        ...
        /* 200 (başarı) VEYA 3xx (redirect — Cloudflare 301/302) kabul. */
        if (k0 == '2' && rx[kod_off + 1] == '0' && rx[kod_off + 2] == '0') kabul = 1;
        if (k0 == '3') kabul = 1;
        /* Durum kodunu da hex olarak bas (3 byte). */
        sys(5, (unsigned long)(uintptr_t)"USERHTTP KOD=");
        bas_bayt_hex(rx[kod_off]);
        bas_bayt_hex(rx[kod_off + 1]);
        bas_bayt_hex(rx[kod_off + 2]);
        sys(7, 0);
    }

    if (kabul) {
        sys(5, (unsigned long)(uintptr_t)"USERHTTP OK");
    } else {
        /* HTTP/1.x bulundu ama beklenen kod değil — yine de yanıt alındı. */
        sys(5, (unsigned long)(uintptr_t)"USERHTTP YANIT ALINDI");
    }
    sys(7, 0);
    for (;;) { }
}
...
int main(void) {
    kdl_yazdir_metin("USERHTTP BASLA");
    kdl_yazdir_satir();
