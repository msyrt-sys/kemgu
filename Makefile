	...
	@if command -v qemu-system-aarch64 > /dev/null 2>&1; then \
		...
		if grep -q "KEMGU KEM-OS OK" $(BUILD)/kem_os.out \
		   ...
		   && grep -q "PING CANLI" $(BUILD)/kem_os.out; then \
			echo "Faz-A TAM .kem-native OS gecti: [1..5] + MMU FAULT/CEVIRI + TRAP KARAR + TIMER TIK + PREEMPT + EL0 SYSCALL + IZOLASYON + LINCHPIN + UART RX + FS SYSCALL + SHELL + SPAWN + ADRES ALANI + SUREC IZOLASYON + ELF YUKLE + W^X + DISK/FS RW + NET DEV/ARP + PING CANLI (SAF-.kem)."; \
		else \
