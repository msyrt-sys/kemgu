...
- [x] **C4.5 (bu commit):** Adim 7 yarım — Tamam strengthen + kismi proof. SmallStep: 8 Tamam'a h_no_fault_target. NoFault: step_fault_preserves Tamam case'leri **FULL** (8 case `exact h_no_fault_target`); typed_no_fault refl case **FULL** (KonfTipliFull.fault). sorry: 22 (+6 geçici — eski 2 statement-only → 8 yapısal sub-sorry, Aile 2 + preservation_konfTipli yolu netlesti).
