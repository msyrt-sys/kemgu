...
- adim_korunum: BIRLESIK korunum (tek yonlu guclu induksiyon) — eski
  ...
  ileri lemma. 21/21 case TAM ISPATLI (Onarim v3 kapanis — sorry 0:
  id-anahtarlama + Yol-B + FIX-F/G + adim_odak_yuku kompozisyonu).
- typed_no_fault: adim_korunum'un StepStar kosesi — TAM
  (dongu YOK, ileri induksiyon; sorry YOK).
...
theorem step_fault_preserves_typed
  ...
  | sGuvensizCong S S' S1 S1' ts1 ts2 ts2' ctx ctx' e e' h_t h_if h_S1 h_inner h_t1' h_tid h_if' h_yan h_S' ih =>
      ...
      have h_nf1 : (ifadeyleKonf S ts1 ts2 ctx e).fault = none := by
        simpa [ifadeyleKonf] using h_no_fault
      simpa using ih h_konf1 h_nf1
  -- D-332: sEgerCong — diger cong'larla birebir ayni desen (odaklama + IH).
  | sEgerCong S S' S1 S1' ts1 ts2 ts2' ctx ctx' k k' d y h_t h_if h_S1 h_inner h_t1' h_tid h_if' h_yan h_S' ih =>
      ...
      obtain ⟨τ', Λ'', Ρ'', h_typed⟩ := h_typed_S.2.1 ctx h_ctx_in
      rw [h_if] at h_typed
      have h_typed_k := typed_eger_kosul h_typed
      have h_konf1 : KonfTipliFull Γ Δ Ρ (ifadeyleKonf S ts1 ts2 ctx k) := by
        show KonfTipliFull Γ Δ Ρ
          { S with thread := ts1 ++ { ctx with ifade := k } :: ts2 }
        exact konfTipliFull_odak Γ Δ Ρ S ts1 ts2 ctx k h_typed_S h_t h_typed_k
          (fun z h => by rw [h_if]; exact HedefVar.eger_kosul k d y z h)
          (fun bb h => by rw [h_if]; exact HedefBolge.eger_kosul k d y bb h)
      have h_nf1 : (ifadeyleKonf S ts1 ts2 ctx k).fault = none := by
        simpa [ifadeyleKonf] using h_no_fault
      simpa using ih h_konf1 h_nf1
  -- D-334: iki topla cong'u da ayni desen.
  | sToplaCongSol S S' S1 S1' ts1 ts2 ts2' ctx ctx' a a' b h_t h_if h_S1 h_inner h_t1' h_tid h_if' h_yan h_S' ih =>
      ...
      have h_konf1 : KonfTipliFull Γ Δ Ρ (ifadeyleKonf S ts1 ts2 ctx a) := by
        show KonfTipliFull Γ Δ Ρ
          { S with thread := ts1 ++ { ctx with ifade := a } :: ts2 }
        exact konfTipliFull_odak Γ Δ Ρ S ts1 ts2 ctx a h_typed_S h_t
          (typed_topla_sol h_typed)
  ...
  | sToplaCongSag S S' S1 S1' ts1 ts2 ts2' ctx ctx' v b b' h_t h_if h_S1 h_inner h_t1' h_tid h_if' h_yan h_S' ih =>
      ...
      obtain ⟨τ', Λ'', Ρ'', h_typed⟩ := h_typed_S.2.1 ctx h_ctx_in
      rw [h_if] at h_typed
      have h_konf1 : KonfTipliFull Γ Δ Ρ (ifadeyleKonf S ts1 ts2 ctx b) := by
        show KonfTipliFull Γ Δ Ρ
          { S with thread := ts1 ++ { ctx with ifade := b } :: ts2 }
        exact konfTipliFull_odak Γ Δ Ρ S ts1 ts2 ctx b h_typed_S h_t
          ⟨Tip.scalar, Λ'', Ρ'', typed_topla_sag h_typed⟩
          (fun z h => by rw [h_if]; exact HedefVar.topla_sag _ b z h)
          (fun bb h => by rw [h_if]; exact HedefBolge.topla_sag _ b bb h)
      have h_nf1 : (ifadeyleKonf S ts1 ts2 ctx b).fault = none := by
        simpa [ifadeyleKonf] using h_no_fault
      simpa using ih h_konf1 h_nf1
  ...
  | sBolCongSol S S' S1 S1' ts1 ts2 ts2' ctx ctx' a a' b h_t h_if h_S1 h_inner h_t1' h_tid h_if' h_yan h_S' ih =>
      ...
      have h_konf1 : KonfTipliFull Γ Δ Ρ (ifadeyleKonf S ts1 ts2 ctx a) := by
        show KonfTipliFull Γ Δ Ρ
          { S with thread := ts1 ++ { ctx with ifade := a } :: ts2 }
        exact konfTipliFull_odak Γ Δ Ρ S ts1 ts2 ctx a h_typed_S h_t
          (typed_bol_sol h_typed)
          (fun z h => by rw [h_if]; exact HedefVar.bol_sol a b z h)
          (fun bb h => by rw [h_if]; exact HedefBolge.bol_sol a b bb h)
      have h_nf1 : (ifadeyleKonf S ts1 ts2 ctx a).fault = none := by
        simpa [ifadeyleKonf] using h_no_fault
      simpa using ih h_konf1 h_nf1
  | sKalanCongSol S S' S1 S1' ts1 ts2 ts2' ctx ctx' a a' b h_t h_if h_S1 h_inner h_t1' h_tid h_if' h_yan h_S' ih =>
      ...
      have h_konf1 : KonfTipliFull Γ Δ Ρ (ifadeyleKonf S ts1 ts2 ctx a) := by
        show KonfTipliFull Γ Δ Ρ
          { S with thread := ts1 ++ { ctx with ifade := a } :: ts2 }
        exact konfTipliFull_odak Γ Δ Ρ S ts1 ts2 ctx a h_typed_S h_t
          (typed_kalan_sol h_typed)
          (fun z h => by rw [h_if]; exact HedefVar.kalan_sol a b z h)
          (fun bb h => by rw [h_if]; exact HedefBolge.kalan_sol a b bb h)
      have h_nf1 : (ifadeyleKonf S ts1 ts2 ctx a).fault = none := by
        simpa [ifadeyleKonf] using h_no_fault
      simpa using ih h_konf1 h_nf1
  | sBolCongSag S S' S1 S1' ts1 ts2 ts2' ctx ctx' v b b' h_t h_if h_S1 h_inner h_t1' h_tid h_if' h_yan h_S' ih =>
      ...
      have h_konf1 : KonfTipliFull Γ Δ Ρ (ifadeyleKonf S ts1 ts2 ctx b) := by
        show KonfTipliFull Γ Δ Ρ
          { S with thread := ts1 ++ { ctx with ifade := b } :: ts2 }
        exact konfTipliFull_odak Γ Δ Ρ S ts1 ts2 ctx b h_typed_S h_t
          ⟨Tip.scalar, Λ'', Ρ'', typed_bol_sag h_typed⟩
          (fun z h => by rw [h_if]; exact HedefVar.bol_sag _ b z h)
          (fun bb h => by rw [h_if]; exact HedefBolge.bol_sag _ b bb h)
      have h_nf1 : (ifadeyleKonf S ts1 ts2 ctx b).fault = none := by
        simpa [ifadeyleKonf] using h_no_fault
      simpa using ih h_konf1 h_nf1
  | sKalanCongSag S S' S1 S1' ts1 ts2 ts2' ctx ctx' v b b' h_t h_if h_S1 h_inner h_t1' h_tid h_if' h_yan h_S' ih =>
      ...
      have h_konf1 : KonfTipliFull Γ Δ Ρ (ifadeyleKonf S ts1 ts2 ctx b) := by
        show KonfTipliFull Γ Δ Ρ
          { S with thread := ts1 ++ { ctx with ifade := b } :: ts2 }
        exact konfTipliFull_odak Γ Δ Ρ S ts1 ts2 ctx b h_typed_S h_t
          ⟨Tip.scalar, Λ'', Ρ'', typed_kalan_sag h_typed⟩
          (fun z h => by rw [h_if]; exact HedefVar.kalan_sag _ b z h)
          (fun bb h => by rw [h_if]; exact HedefBolge.kalan_sag _ b bb h)
      have h_nf1 : (ifadeyleKonf S ts1 ts2 ctx b).fault = none := by
        simpa [ifadeyleKonf] using h_no_fault
      simpa using ih h_konf1 h_nf1
  -- D-335: sEslesCong — diger cong'larla ayni desen.
  | sEslesCong S S' S1 S1' ts1 ts2 ts2' ctx ctx' s s' n d y h_t h_if h_S1 h_inner h_t1' h_tid h_if' h_yan h_S' ih =>
      ...
      obtain ⟨τ', Λ'', Ρ'', h_typed⟩ := h_typed_S.2.1 ctx h_ctx_in
      rw [h_if] at h_typed
      have h_konf1 : KonfTipliFull Γ Δ Ρ (ifadeyleKonf S ts1 ts2 ctx s) := by
        show KonfTipliFull Γ Δ Ρ
          { S with thread := ts1 ++ { ctx with ifade := s } :: ts2 }
        exact konfTipliFull_odak Γ Δ Ρ S ts1 ts2 ctx s h_typed_S h_t
          (typed_esles_skrut h_typed)
  ...
  | sIndeksCong S S' S1 S1' ts1 ts2 ts2' ctx ctx' x e e' h_t h_if h_S1 h_inner h_t1' h_tid h_if' h_yan h_S' ih =>
      ...
      have h_nf1 : (ifadeyleKonf S ts1 ts2 ctx e).fault = none := by
        simpa [ifadeyleKonf] using h_no_fault
      simpa using ih h_konf1 h_nf1
  -- D-337: indeksli yazmanin iki cong'u.
  | sIndeksAtaCongIdx S S' S1 S1' ts1 ts2 ts2' ctx ctx' x idx idx' e h_t h_if h_S1 h_inner h_t1' h_tid h_if' h_yan h_S' ih =>
      ...
      obtain ⟨τ', Λ'', Ρ'', h_typed⟩ := h_typed_S.2.1 ctx h_ctx_in
      rw [h_if] at h_typed
      have h_konf1 : KonfTipliFull Γ Δ Ρ (ifadeyleKonf S ts1 ts2 ctx idx) := by
        show KonfTipliFull Γ Δ Ρ
          { S with thread := ts1 ++ { ctx with ifade := idx } :: ts2 }
        exact konfTipliFull_odak Γ Δ Ρ S ts1 ts2 ctx idx h_typed_S h_t
          (typed_indeks_ata_idx h_typed)
      ...
      have h_nf1 : (ifadeyleKonf S ts1 ts2 ctx idx).fault = none := by
        simpa [ifadeyleKonf] using h_no_fault
      simpa using ih h_konf1 h_nf1
  | sIndeksAtaCongDeg S S' S1 S1' ts1 ts2 ts2' ctx ctx' x v e e' h_t h_if h_S1 h_inner h_t1' h_tid h_if' h_yan h_S' ih =>
      ...
      have h_konf1 : KonfTipliFull Γ Δ Ρ (ifadeyleKonf S ts1 ts2 ctx e) := by
        show KonfTipliFull Γ Δ Ρ
          { S with thread := ts1 ++ { ctx with ifade := e } :: ts2 }
        exact konfTipliFull_odak Γ Δ Ρ S ts1 ts2 ctx e h_typed_S h_t
          ⟨Tip.scalar, Λ'', Ρ'', typed_indeks_ata_deg h_typed⟩
...
theorem adim_odak_yuku
  ...
  | sSeqAtla S S' ts1 ts2 ctx v b h_t h_if h_S' =>
      ...
      · intro y bb h_o h_yz
        exact ⟨h_o, fun h => h⟩
  -- D-332: sEgerSec — secilen dal odaga girer; ortamlar DEGISMEZ
  -- (l_eger/r_eger cikisi = kosulun cikisi, kosul burada `sabit v`).
  | sEgerSec S S' ts1 ts2 ctx v d y alindi h_t h_if h_dal h_S' =>
      intro h_konf ts1o ts2o ctxo ts2o' ctxo' h1 h2 h_yano
      subst h_S'
      have h_ne : ({ ctx with ifade := if alindi then d else y } : ThreadCtx)
          ≠ ctx := by
        intro h_e
        have h_i : (if alindi then d else y) = ctx.ifade :=
          congrArg ThreadCtx.ifade h_e
        rw [h_if] at h_i
        cases alindi with
        | true  => exact eger_ne_dogru (Ifade.sabit v) d y (by simpa using h_i.symm)
        | false => exact eger_ne_yanlis (Ifade.sabit v) d y (by simpa using h_i.symm)
      have h2' : ts1 ++ ({ ctx with ifade := if alindi then d else y } : ThreadCtx)
          :: ts2 = ts1o ++ ctxo' :: ts2o' := h2
      obtain ⟨h_e1, h_e2, h_e3, h_e4, h_e5⟩ :=
        cerrah_kilit ts1o ts1 (h_t.symm.trans h1) h2' (Or.inl rfl) h_yano h_ne
      rw [h_e2, h_e3]
      ...
      · intro τo Λo Ρoo h_ty
        rw [h_if] at h_ty
        exact ⟨Λo, Ρoo, typed_eger_dal h_ty alindi,
          lineerKucuk_refl _, bolgeIliski_refl _⟩
      · intro y' bb h_o h_yz
        exact ⟨h_o, fun h => h⟩
  -- D-338: sBolTamam — sToplaTamam ile ayni sekil (olay uretse de
  -- ortam/hedef acisindan fark yok; odak yuku ize bakmaz).
  | sBolTamam S S' ts1 ts2 ctx n1 n2 h_t h_if h_S' =>
      intro h_konf ts1o ts2o ctxo ts2o' ctxo' h1 h2 h_yano
      subst h_S'
      have h_ne : ({ ctx with ifade := Ifade.sabit (.skaler (n1 / n2)) } : ThreadCtx)
          ≠ ctx := by
        intro h_e
        have h_i : Ifade.sabit (Deger.skaler (n1 / n2)) = ctx.ifade :=
          congrArg ThreadCtx.ifade h_e
        rw [h_if] at h_i
        nomatch h_i
      have h2' : ts1 ++ ({ ctx with ifade := Ifade.sabit (.skaler (n1 / n2)) }
          : ThreadCtx) :: ts2 = ts1o ++ ctxo' :: ts2o' := h2
      obtain ⟨h_e1, h_e2, h_e3, h_e4, h_e5⟩ :=
        cerrah_kilit ts1o ts1 (h_t.symm.trans h1) h2' (Or.inl rfl) h_yano h_ne
      rw [h_e2, h_e3]
      refine ⟨h_ne, ?_, ?_⟩
      · intro τo Λo Ρoo h_ty
        rw [h_if] at h_ty
        exact ⟨Λo, Ρoo, typed_bol_atla h_ty,
          lineerKucuk_refl _, bolgeIliski_refl _⟩
      · intro y bb h_o h_yz
        exact ⟨h_o, fun h => h⟩
  | sKalanTamam S S' ts1 ts2 ctx n1 n2 h_t h_if h_S' =>
      ...
      · intro τo Λo Ρoo h_ty
        rw [h_if] at h_ty
        exact ⟨Λo, Ρoo, typed_kalan_atla h_ty,
          lineerKucuk_refl _, bolgeIliski_refl _⟩
      · intro y bb h_o h_yz
        exact ⟨h_o, fun h => h⟩
  -- D-334: sToplaTamam — sSeqAtla ile ayni sekil (olaysiz, ortam sabit).
  | sToplaTamam S S' ts1 ts2 ctx n1 n2 h_t h_if h_S' =>
      intro h_konf ts1o ts2o ctxo ts2o' ctxo' h1 h2 h_yano
      subst h_S'
      have h_ne : ({ ctx with ifade := Ifade.sabit (.skaler (n1 + n2)) } : ThreadCtx)
      ...
      · intro τo Λo Ρoo h_ty
        rw [h_if] at h_ty
        exact ⟨Λo, Ρoo, typed_topla_atla h_ty,
          lineerKucuk_refl _, bolgeIliski_refl _⟩
      · intro y bb h_o h_yz
        exact ⟨h_o, fun h => h⟩
  | sCarpTamam S S' ts1 ts2 ctx n1 n2 h_t h_if h_S' =>
      ...
      · intro τo Λo Ρoo h_ty
        rw [h_if] at h_ty
        exact ⟨Λo, Ρoo, typed_carp_atla h_ty,
          lineerKucuk_refl _, bolgeIliski_refl _⟩
      · intro y bb h_o h_yz
        exact ⟨h_o, fun h => h⟩
  -- D-335: sIkenAc — acilma; ortamlar DEGISMEZ, hedefler acilmis terimden
  -- orijinaline geri esler (iken_kosul / iken_govde).
  | sIkenAc S S' ts1 ts2 ctx k g h_t h_if h_S' =>
      intro h_konf ts1o ts2o ctxo ts2o' ctxo' h1 h2 h_yano
      subst h_S'
      have h_ne : ({ ctx with ifade := Ifade.eger k (Ifade.seq g (Ifade.iken k g)) (Ifade.sabit Deger.birim) } : ThreadCtx) ≠ ctx := by
        intro h_e
        have h_i : Ifade.eger k (Ifade.seq g (Ifade.iken k g)) (Ifade.sabit Deger.birim) = ctx.ifade :=
          congrArg ThreadCtx.ifade h_e
        rw [h_if] at h_i
        nomatch h_i
      have h2' : ts1 ++ ({ ctx with ifade := Ifade.eger k (Ifade.seq g (Ifade.iken k g)) (Ifade.sabit Deger.birim) } : ThreadCtx) :: ts2 = ts1o ++ ctxo' :: ts2o' := h2
      obtain ⟨h_e1, h_e2, h_e3, h_e4, h_e5⟩ :=
        cerrah_kilit ts1o ts1 (h_t.symm.trans h1) h2' (Or.inl rfl) h_yano h_ne
      rw [h_e2, h_e3]
      refine ⟨h_ne, ?_, ?_⟩
      · intro τo Λo Ρoo h_ty
        rw [h_if] at h_ty
        exact ⟨Λo, Ρoo, typed_iken_ac h_ty,
          lineerKucuk_refl _, bolgeIliski_refl _⟩
      · intro y bb h_o h_yz
        exact ⟨h_o, fun h => h⟩
  -- D-335: sEslesSec — sEgerSec sekli.
  | sEslesSec S S' ts1 ts2 ctx m n d y tuttu h_t h_if h_dal h_S' =>
      intro h_konf ts1o ts2o ctxo ts2o' ctxo' h1 h2 h_yano
      subst h_S'
      have h_ne : ({ ctx with ifade := if tuttu then d else y } : ThreadCtx)
          ≠ ctx := by
        intro h_e
        have h_i : (if tuttu then d else y) = ctx.ifade :=
          congrArg ThreadCtx.ifade h_e
        rw [h_if] at h_i
        cases tuttu with
        | true  => exact esles_ne_eslesen (Ifade.sabit (.skaler m)) n d y
                     (by simpa using h_i.symm)
        | false => exact esles_ne_kalan (Ifade.sabit (.skaler m)) n d y
                     (by simpa using h_i.symm)
      have h2' : ts1 ++ ({ ctx with ifade := if tuttu then d else y } : ThreadCtx)
          :: ts2 = ts1o ++ ctxo' :: ts2o' := h2
      obtain ⟨h_e1, h_e2, h_e3, h_e4, h_e5⟩ :=
        cerrah_kilit ts1o ts1 (h_t.symm.trans h1) h2' (Or.inl rfl) h_yano h_ne
      rw [h_e2, h_e3]
  ...
  | sIndeksOku S S' ts1 ts2 ctx x i b n h_t h_if h_b h_n h_S' =>
      ...
      · intro y bb h_o h_yz
        exact ⟨h_o, fun h => h⟩
  -- D-337: sIndeksYaz — sAtamaTamam sekli (yazma; hedef x).
  | sIndeksYaz S S' ts1 ts2 ctx x i n b h_t h_if h_b h_owner h_S' =>
      intro h_konf ts1o ts2o ctxo ts2o' ctxo' h1 h2 h_yano
      subst h_S'
      have h_ne : ({ ctx with ifade := Ifade.sabit Deger.birim } : ThreadCtx)
      ...
      · intro τo Λo Ρoo h_ty
        rw [h_if] at h_ty
        exact ⟨Λo, Ρoo, typed_indeks_yaz h_ty,
          lineerKucuk_refl _, bolgeIliski_refl _⟩
      · intro y bb h_o h_yz
        exact ⟨h_o, fun h => h⟩
  | sGuvensizAtla S S' ts1 ts2 ctx v h_t h_if h_S' =>
      ...
      · intro τo Λo Ρoo h_ty
        ...
        | HasType.t_atama _ _ _ _ τx h_gx hte,
          ...
          exact ⟨Λe', Ρe',
            ...
            h_ke, h_ie⟩
      · exact h_B
  -- D-332: sEgerCong odak-yuku. Dallar icin ORTAM TASIMASI GEREKMEZ:
  -- LineerNotr/RegionNotr sozdizimseldir (yeni ortamda aynen gecerli),
  -- r_eger'in Ρ-bagli dal premise'leri iliski-transport + notr-kimlik
  -- ile yeni Ρk''de yeniden kurulur.
  | sEgerCong S S' S1 S1' ts1 ts2 ts2' ctx ctx' k k' d y h_t h_if h_S1 h_inner h_t1' h_tid h_if' h_yan h_S' ih =>
      intro h_konf ts1o ts2o ctxo ts2o' ctxo' h1 h2 h_yano
      subst h_S1
      subst h_S'
      ...
      obtain ⟨τ0, Λ0, Ρ0, h_ty0⟩ := h_konf.2.1 ctx h_ctx_in
      rw [h_if] at h_ty0
      have h_konf1 : KonfTipliFull Γ Δ Ρ (ifadeyleKonf S ts1 ts2 ctx k) :=
        konfTipliFull_odak Γ Δ Ρ S ts1 ts2 ctx k h_konf h_t
          (typed_eger_kosul h_ty0)
          (fun z h => by rw [h_if]; exact HedefVar.eger_kosul k d y z h)
          (fun bb h => by rw [h_if]; exact HedefBolge.eger_kosul k d y bb h)
      obtain ⟨h_nei, h_A, h_B⟩ :=
        ih h_konf1 ts1 ts2 { ctx with ifade := k } ts2' ctx' rfl h_t1' h_yan
      have h_ne : ({ ctx' with ifade := Ifade.eger k' d y } : ThreadCtx)
          ≠ ctx := by
        intro h_e
        have h_if2x := congrArg ThreadCtx.ifade h_e
        have h_if2 : Ifade.eger k' d y = ctx.ifade := h_if2x
        have h_lin2x := congrArg ThreadCtx.lineer h_e
        have h_lin2 : ctx'.lineer = ctx.lineer := h_lin2x
        rw [h_if] at h_if2
        injection h_if2 with h_kk _ _
        apply h_nei
        show (⟨ctx'.tid, ctx'.ifade, ctx'.lineer⟩ : ThreadCtx)
            = ⟨ctx.tid, k, ctx.lineer⟩
        rw [h_tid, h_if', h_kk, h_lin2]
      have h2' : ts1 ++ ({ ctx' with ifade := Ifade.eger k' d y } : ThreadCtx)
          :: ts2' = ts1o ++ ctxo' :: ts2o' := h2
      obtain ⟨h_e1, h_e2, h_e3, h_e4, h_e5⟩ :=
        cerrah_kilit ts1o ts1 (h_t.symm.trans h1) h2' h_yan h_yano h_ne
      rw [h_e2, h_e3]
      ...
      · intro τo Λo Ρoo h_ty
        ...
        obtain ⟨ht, hl, hr⟩ := h_ty
        match ht, hl, hr with
        | HasType.t_eger _ _ _ _ _ τk _ htk htd hty,
          LineerTamam.l_eger _ _ Λk _ _ _ hlk h_nd h_ny,
          RegionTamam.r_eger _ _ Ρk _ _ _ hrk h_rnd h_rny hrd hry =>
          obtain ⟨Λk', Ρk', h_tyk', h_kk, h_ik⟩ := h_A τk Λk Ρk ⟨htk, hlk, hrk⟩
          rw [h_if'] at h_tyk'
          obtain ⟨Ρdn, h_rd', _⟩ := regionTamam_iliski_transport hrd Ρk' h_ik
          obtain ⟨Ρyn, h_ry', _⟩ := regionTamam_iliski_transport hry Ρk' h_ik
          rw [regionNotr_cikis_esit h_rd' h_rnd] at h_rd'
          rw [regionNotr_cikis_esit h_ry' h_rny] at h_ry'
          exact ⟨Λk', Ρk',
            ⟨HasType.t_eger _ _ _ _ _ τk _ h_tyk'.hasType htd hty,
             LineerTamam.l_eger _ _ _ k' d y h_tyk'.lineerOK h_nd h_ny,
             RegionTamam.r_eger _ _ _ k' d y h_tyk'.regionOK h_rnd h_rny
               h_rd' h_ry'⟩,
            h_kk, h_ik⟩
      · exact h_B
  -- D-338: bolme cong'lari — topla ile birebir ayni.
  | sBolCongSol S S' S1 S1' ts1 ts2 ts2' ctx ctx' a a' b h_t h_if h_S1 h_inner h_t1' h_tid h_if' h_yan h_S' ih =>
      intro h_konf ts1o ts2o ctxo ts2o' ctxo' h1 h2 h_yano
      subst h_S1
      subst h_S'
      ...
      obtain ⟨h_nei, h_A, h_B⟩ :=
        ih h_konf1 ts1 ts2 { ctx with ifade := a } ts2' ctx' rfl h_t1' h_yan
      have h_ne : ({ ctx' with ifade := Ifade.bol a' b } : ThreadCtx) ≠ ctx := by
        intro h_e
        have h_if2 : Ifade.bol a' b = ctx.ifade := congrArg ThreadCtx.ifade h_e
        have h_lin2x := congrArg ThreadCtx.lineer h_e
        have h_lin2 : ctx'.lineer = ctx.lineer := h_lin2x
        rw [h_if] at h_if2
        injection h_if2 with h_aa _
        ...
        show (⟨ctx'.tid, ctx'.ifade, ctx'.lineer⟩ : ThreadCtx)
            = ⟨ctx.tid, a, ctx.lineer⟩
        rw [h_tid, h_if', h_aa, h_lin2]
      have h2' : ts1 ++ ({ ctx' with ifade := Ifade.bol a' b } : ThreadCtx)
          :: ts2' = ts1o ++ ctxo' :: ts2o' := h2
      obtain ⟨h_e1, h_e2, h_e3, h_e4, h_e5⟩ :=
        cerrah_kilit ts1o ts1 (h_t.symm.trans h1) h2' h_yan h_yano h_ne
      rw [h_e2, h_e3]
      ...
      · intro τo Λo Ρoo h_ty
        ...
        obtain ⟨ht, hl, hr⟩ := h_ty
        match ht, hl, hr with
        | HasType.t_bol _ _ _ _ hta htb,
          LineerTamam.l_bol _ _ Λa _ _ _ hla hlb,
          RegionTamam.r_bol _ _ Ρa _ _ _ hra hrb =>
          obtain ⟨Λa', Ρa', h_tya', h_ka, h_ia⟩ :=
            h_A Tip.scalar Λa Ρa ⟨hta, hla, hra⟩
          rw [h_if'] at h_tya'
          obtain ⟨Λb', h_lb', h_kb⟩ := lineerTamam_kucuk_transport hlb Λa' h_ka
          obtain ⟨Ρb', h_rb', h_ib⟩ := regionTamam_iliski_transport hrb Ρa' h_ia
          exact ⟨Λb', Ρb',
            ⟨HasType.t_bol _ _ _ _ h_tya'.hasType htb,
             LineerTamam.l_bol _ _ _ _ a' b h_tya'.lineerOK h_lb',
             RegionTamam.r_bol _ _ _ _ a' b h_tya'.regionOK h_rb'⟩,
            h_kb, h_ib⟩
      · exact h_B
  | sKalanCongSol S S' S1 S1' ts1 ts2 ts2' ctx ctx' a a' b h_t h_if h_S1 h_inner h_t1' h_tid h_if' h_yan h_S' ih =>
      intro h_konf ts1o ts2o ctxo ts2o' ctxo' h1 h2 h_yano
      ...
      have h_konf1 : KonfTipliFull Γ Δ Ρ (ifadeyleKonf S ts1 ts2 ctx a) :=
        konfTipliFull_odak Γ Δ Ρ S ts1 ts2 ctx a h_konf h_t
          (typed_kalan_sol h_ty0)
          (fun z h => by rw [h_if]; exact HedefVar.kalan_sol a b z h)
          (fun bb h => by rw [h_if]; exact HedefBolge.kalan_sol a b bb h)
      obtain ⟨h_nei, h_A, h_B⟩ :=
        ih h_konf1 ts1 ts2 { ctx with ifade := a } ts2' ctx' rfl h_t1' h_yan
      have h_ne : ({ ctx' with ifade := Ifade.kalan a' b } : ThreadCtx) ≠ ctx := by
        intro h_e
        have h_if2 : Ifade.kalan a' b = ctx.ifade := congrArg ThreadCtx.ifade h_e
        have h_lin2x := congrArg ThreadCtx.lineer h_e
        have h_lin2 : ctx'.lineer = ctx.lineer := h_lin2x
        rw [h_if] at h_if2
        injection h_if2 with h_aa _
        ...
        show (⟨ctx'.tid, ctx'.ifade, ctx'.lineer⟩ : ThreadCtx)
            = ⟨ctx.tid, a, ctx.lineer⟩
        rw [h_tid, h_if', h_aa, h_lin2]
      have h2' : ts1 ++ ({ ctx' with ifade := Ifade.kalan a' b } : ThreadCtx)
          :: ts2' = ts1o ++ ctxo' :: ts2o' := h2
      obtain ⟨h_e1, h_e2, h_e3, h_e4, h_e5⟩ :=
        cerrah_kilit ts1o ts1 (h_t.symm.trans h1) h2' h_yan h_yano h_ne
      rw [h_e2, h_e3]
      ...
      · intro τo Λo Ρoo h_ty
        ...
        obtain ⟨ht, hl, hr⟩ := h_ty
        match ht, hl, hr with
        | HasType.t_kalan _ _ _ _ hta htb,
          LineerTamam.l_kalan _ _ Λa _ _ _ hla hlb,
          RegionTamam.r_kalan _ _ Ρa _ _ _ hra hrb =>
          obtain ⟨Λa', Ρa', h_tya', h_ka, h_ia⟩ :=
            h_A Tip.scalar Λa Ρa ⟨hta, hla, hra⟩
          rw [h_if'] at h_tya'
          obtain ⟨Λb', h_lb', h_kb⟩ := lineerTamam_kucuk_transport hlb Λa' h_ka
          obtain ⟨Ρb', h_rb', h_ib⟩ := regionTamam_iliski_transport hrb Ρa' h_ia
          exact ⟨Λb', Ρb',
            ⟨HasType.t_kalan _ _ _ _ h_tya'.hasType htb,
             LineerTamam.l_kalan _ _ _ _ a' b h_tya'.lineerOK h_lb',
             RegionTamam.r_kalan _ _ _ _ a' b h_tya'.regionOK h_rb'⟩,
            h_kb, h_ib⟩
      · exact h_B
  | sBolCongSag S S' S1 S1' ts1 ts2 ts2' ctx ctx' v b b' h_t h_if h_S1 h_inner h_t1' h_tid h_if' h_yan h_S' ih =>
      intro h_konf ts1o ts2o ctxo ts2o' ctxo' h1 h2 h_yano
      ...
      obtain ⟨τ0, Λ0, Ρ0, h_ty0⟩ := h_konf.2.1 ctx h_ctx_in
      rw [h_if] at h_ty0
      have h_konf1 : KonfTipliFull Γ Δ Ρ (ifadeyleKonf S ts1 ts2 ctx b) :=
        konfTipliFull_odak Γ Δ Ρ S ts1 ts2 ctx b h_konf h_t
          ⟨Tip.scalar, Λ0, Ρ0, typed_bol_sag h_ty0⟩
          (fun z h => by rw [h_if]; exact HedefVar.bol_sag _ b z h)
          (fun bb h => by rw [h_if]; exact HedefBolge.bol_sag _ b bb h)
      obtain ⟨h_nei, h_A, h_B⟩ :=
        ih h_konf1 ts1 ts2 { ctx with ifade := b } ts2' ctx' rfl h_t1' h_yan
      have h_ne : ({ ctx' with ifade := Ifade.bol (.sabit v) b' } : ThreadCtx)
          ≠ ctx := by
        intro h_e
        have h_if2 : Ifade.bol (.sabit v) b' = ctx.ifade :=
          congrArg ThreadCtx.ifade h_e
        have h_lin2x := congrArg ThreadCtx.lineer h_e
        have h_lin2 : ctx'.lineer = ctx.lineer := h_lin2x
        rw [h_if] at h_if2
        injection h_if2 with _ h_bb
        apply h_nei
        show (⟨ctx'.tid, ctx'.ifade, ctx'.lineer⟩ : ThreadCtx)
            = ⟨ctx.tid, b, ctx.lineer⟩
        rw [h_tid, h_if', h_bb, h_lin2]
      have h2' : ts1 ++ ({ ctx' with ifade := Ifade.bol (.sabit v) b' } : ThreadCtx)
          :: ts2' = ts1o ++ ctxo' :: ts2o' := h2
      obtain ⟨h_e1, h_e2, h_e3, h_e4, h_e5⟩ :=
        cerrah_kilit ts1o ts1 (h_t.symm.trans h1) h2' h_yan h_yano h_ne
      rw [h_e2, h_e3]
      ...
      · intro τo Λo Ρoo h_ty
        ...
        match ht, hl, hr with
        | HasType.t_bol _ _ _ _ hta htb,
          LineerTamam.l_bol _ _ _ _ _ _ hla hlb,
          RegionTamam.r_bol _ _ _ _ _ _ hra hrb =>
          cases hla; cases hra
          obtain ⟨Λb', Ρb', h_tyb', h_kb, h_ib⟩ :=
            h_A Tip.scalar Λo Ρoo ⟨htb, hlb, hrb⟩
          rw [h_if'] at h_tyb'
          exact ⟨Λb', Ρb',
            ⟨HasType.t_bol _ _ _ _ hta h_tyb'.hasType,
             LineerTamam.l_bol _ _ _ _ (.sabit v) b'
               (LineerTamam.l_sabit _ _ v) h_tyb'.lineerOK,
             RegionTamam.r_bol _ _ _ _ (.sabit v) b'
  ...
  | sKalanCongSag S S' S1 S1' ts1 ts2 ts2' ctx ctx' v b b' h_t h_if h_S1 h_inner h_t1' h_tid h_if' h_yan h_S' ih =>
      ...
      have h_konf1 : KonfTipliFull Γ Δ Ρ (ifadeyleKonf S ts1 ts2 ctx b) :=
        konfTipliFull_odak Γ Δ Ρ S ts1 ts2 ctx b h_konf h_t
          ⟨Tip.scalar, Λ0, Ρ0, typed_kalan_sag h_ty0⟩
          (fun z h => by rw [h_if]; exact HedefVar.kalan_sag _ b z h)
          (fun bb h => by rw [h_if]; exact HedefBolge.kalan_sag _ b bb h)
      obtain ⟨h_nei, h_A, h_B⟩ :=
        ih h_konf1 ts1 ts2 { ctx with ifade := b } ts2' ctx' rfl h_t1' h_yan
      have h_ne : ({ ctx' with ifade := Ifade.kalan (.sabit v) b' } : ThreadCtx)
          ≠ ctx := by
        intro h_e
        have h_if2 : Ifade.kalan (.sabit v) b' = ctx.ifade :=
          congrArg ThreadCtx.ifade h_e
        have h_lin2x := congrArg ThreadCtx.lineer h_e
        have h_lin2 : ctx'.lineer = ctx.lineer := h_lin2x
        rw [h_if] at h_if2
        ...
        show (⟨ctx'.tid, ctx'.ifade, ctx'.lineer⟩ : ThreadCtx)
            = ⟨ctx.tid, b, ctx.lineer⟩
        rw [h_tid, h_if', h_bb, h_lin2]
      have h2' : ts1 ++ ({ ctx' with ifade := Ifade.kalan (.sabit v) b' } : ThreadCtx)
          :: ts2' = ts1o ++ ctxo' :: ts2o' := h2
      obtain ⟨h_e1, h_e2, h_e3, h_e4, h_e5⟩ :=
        cerrah_kilit ts1o ts1 (h_t.symm.trans h1) h2' h_yan h_yano h_ne
      rw [h_e2, h_e3]
      ...
      · intro τo Λo Ρoo h_ty
        ...
        | HasType.t_kalan _ _ _ _ hta htb,
          ...
          rw [h_if'] at h_tyb'
          exact ⟨Λb', Ρb',
            ⟨HasType.t_kalan _ _ _ _ hta h_tyb'.hasType,
             LineerTamam.l_kalan _ _ _ _ (.sabit v) b'
               (LineerTamam.l_sabit _ _ v) h_tyb'.lineerOK,
             RegionTamam.r_kalan _ _ _ _ (.sabit v) b'
               (RegionTamam.r_sabit _ _ v) h_tyb'.regionOK⟩,
            h_kb, h_ib⟩
      · exact h_B
  -- D-334: topla cong'lari — sSeqCong deseninin aynisi. Sag kolda sol
  -- operand `sabit v` oldugu icin `typed_topla_sag` dogrudan tip verir.
  | sToplaCongSol S S' S1 S1' ts1 ts2 ts2' ctx ctx' a a' b h_t h_if h_S1 h_inner h_t1' h_tid h_if' h_yan h_S' ih =>
      intro h_konf ts1o ts2o ctxo ts2o' ctxo' h1 h2 h_yano
      subst h_S1
      subst h_S'
      ...
      have h_konf1 : KonfTipliFull Γ Δ Ρ (ifadeyleKonf S ts1 ts2 ctx a) :=
        konfTipliFull_odak Γ Δ Ρ S ts1 ts2 ctx a h_konf h_t
          (typed_topla_sol h_ty0)
          (fun z h => by rw [h_if]; exact HedefVar.topla_sol a b z h)
          (fun bb h => by rw [h_if]; exact HedefBolge.topla_sol a b bb h)
      obtain ⟨h_nei, h_A, h_B⟩ :=
        ih h_konf1 ts1 ts2 { ctx with ifade := a } ts2' ctx' rfl h_t1' h_yan
      have h_ne : ({ ctx' with ifade := Ifade.topla a' b } : ThreadCtx) ≠ ctx := by
        intro h_e
        have h_if2 : Ifade.topla a' b = ctx.ifade := congrArg ThreadCtx.ifade h_e
        have h_lin2x := congrArg ThreadCtx.lineer h_e
        have h_lin2 : ctx'.lineer = ctx.lineer := h_lin2x
        rw [h_if] at h_if2
        injection h_if2 with h_aa _
        ...
        show (⟨ctx'.tid, ctx'.ifade, ctx'.lineer⟩ : ThreadCtx)
            = ⟨ctx.tid, a, ctx.lineer⟩
        rw [h_tid, h_if', h_aa, h_lin2]
      have h2' : ts1 ++ ({ ctx' with ifade := Ifade.topla a' b } : ThreadCtx)
          :: ts2' = ts1o ++ ctxo' :: ts2o' := h2
      obtain ⟨h_e1, h_e2, h_e3, h_e4, h_e5⟩ :=
        cerrah_kilit ts1o ts1 (h_t.symm.trans h1) h2' h_yan h_yano h_ne
      rw [h_e2, h_e3]
      ...
      · intro τo Λo Ρoo h_ty
        ...
        | HasType.t_topla _ _ _ _ hta htb,
          ...
          obtain ⟨Ρb', h_rb', h_ib⟩ := regionTamam_iliski_transport hrb Ρa' h_ia
          exact ⟨Λb', Ρb',
            ⟨HasType.t_topla _ _ _ _ h_tya'.hasType htb,
             LineerTamam.l_topla _ _ _ _ a' b h_tya'.lineerOK h_lb',
             RegionTamam.r_topla _ _ _ _ a' b h_tya'.regionOK h_rb'⟩,
            h_kb, h_ib⟩
      · exact h_B
  | sCarpCongSol S S' S1 S1' ts1 ts2 ts2' ctx ctx' a a' b h_t h_if h_S1 h_inner h_t1' h_tid h_if' h_yan h_S' ih =>
      intro h_konf ts1o ts2o ctxo ts2o' ctxo' h1 h2 h_yano
      ...
      have h_konf1 : KonfTipliFull Γ Δ Ρ (ifadeyleKonf S ts1 ts2 ctx a) :=
        konfTipliFull_odak Γ Δ Ρ S ts1 ts2 ctx a h_konf h_t
          (typed_carp_sol h_ty0)
          (fun z h => by rw [h_if]; exact HedefVar.carp_sol a b z h)
          (fun bb h => by rw [h_if]; exact HedefBolge.carp_sol a b bb h)
      obtain ⟨h_nei, h_A, h_B⟩ :=
        ih h_konf1 ts1 ts2 { ctx with ifade := a } ts2' ctx' rfl h_t1' h_yan
      have h_ne : ({ ctx' with ifade := Ifade.carp a' b } : ThreadCtx) ≠ ctx := by
        intro h_e
        have h_if2 : Ifade.carp a' b = ctx.ifade := congrArg ThreadCtx.ifade h_e
        have h_lin2x := congrArg ThreadCtx.lineer h_e
        have h_lin2 : ctx'.lineer = ctx.lineer := h_lin2x
        rw [h_if] at h_if2
        injection h_if2 with h_aa _
        ...
        show (⟨ctx'.tid, ctx'.ifade, ctx'.lineer⟩ : ThreadCtx)
            = ⟨ctx.tid, a, ctx.lineer⟩
        rw [h_tid, h_if', h_aa, h_lin2]
      have h2' : ts1 ++ ({ ctx' with ifade := Ifade.carp a' b } : ThreadCtx)
          :: ts2' = ts1o ++ ctxo' :: ts2o' := h2
      obtain ⟨h_e1, h_e2, h_e3, h_e4, h_e5⟩ :=
        cerrah_kilit ts1o ts1 (h_t.symm.trans h1) h2' h_yan h_yano h_ne
      rw [h_e2, h_e3]
      ...
      · intro τo Λo Ρoo h_ty
        ...
        | HasType.t_carp _ _ _ _ hta htb,
          ...
          obtain ⟨Ρb', h_rb', h_ib⟩ := regionTamam_iliski_transport hrb Ρa' h_ia
          exact ⟨Λb', Ρb',
            ⟨HasType.t_carp _ _ _ _ h_tya'.hasType htb,
             LineerTamam.l_carp _ _ _ _ a' b h_tya'.lineerOK h_lb',
             RegionTamam.r_carp _ _ _ _ a' b h_tya'.regionOK h_rb'⟩,
            h_kb, h_ib⟩
      · exact h_B
  | sToplaCongSag S S' S1 S1' ts1 ts2 ts2' ctx ctx' v b b' h_t h_if h_S1 h_inner h_t1' h_tid h_if' h_yan h_S' ih =>
      intro h_konf ts1o ts2o ctxo ts2o' ctxo' h1 h2 h_yano
      ...
      have h_konf1 : KonfTipliFull Γ Δ Ρ (ifadeyleKonf S ts1 ts2 ctx b) :=
        konfTipliFull_odak Γ Δ Ρ S ts1 ts2 ctx b h_konf h_t
          ⟨Tip.scalar, Λ0, Ρ0, typed_topla_sag h_ty0⟩
          (fun z h => by rw [h_if]; exact HedefVar.topla_sag _ b z h)
          (fun bb h => by rw [h_if]; exact HedefBolge.topla_sag _ b bb h)
      obtain ⟨h_nei, h_A, h_B⟩ :=
        ih h_konf1 ts1 ts2 { ctx with ifade := b } ts2' ctx' rfl h_t1' h_yan
      have h_ne : ({ ctx' with ifade := Ifade.topla (.sabit v) b' } : ThreadCtx)
          ≠ ctx := by
        intro h_e
        have h_if2 : Ifade.topla (.sabit v) b' = ctx.ifade :=
          congrArg ThreadCtx.ifade h_e
        have h_lin2x := congrArg ThreadCtx.lineer h_e
        have h_lin2 : ctx'.lineer = ctx.lineer := h_lin2x
        rw [h_if] at h_if2
        ...
        show (⟨ctx'.tid, ctx'.ifade, ctx'.lineer⟩ : ThreadCtx)
            = ⟨ctx.tid, b, ctx.lineer⟩
        rw [h_tid, h_if', h_bb, h_lin2]
      have h2' : ts1 ++ ({ ctx' with ifade := Ifade.topla (.sabit v) b' } : ThreadCtx)
          :: ts2' = ts1o ++ ctxo' :: ts2o' := h2
      obtain ⟨h_e1, h_e2, h_e3, h_e4, h_e5⟩ :=
        cerrah_kilit ts1o ts1 (h_t.symm.trans h1) h2' h_yan h_yano h_ne
      rw [h_e2, h_e3]
      ...
      · intro τo Λo Ρoo h_ty
        ...
        | HasType.t_topla _ _ _ _ hta htb,
          ...
          rw [h_if'] at h_tyb'
          exact ⟨Λb', Ρb',
            ⟨HasType.t_topla _ _ _ _ hta h_tyb'.hasType,
             LineerTamam.l_topla _ _ _ _ (.sabit v) b'
               (LineerTamam.l_sabit _ _ v) h_tyb'.lineerOK,
             RegionTamam.r_topla _ _ _ _ (.sabit v) b'
               (RegionTamam.r_sabit _ _ v) h_tyb'.regionOK⟩,
            h_kb, h_ib⟩
      · exact h_B
  | sCarpCongSag S S' S1 S1' ts1 ts2 ts2' ctx ctx' v b b' h_t h_if h_S1 h_inner h_t1' h_tid h_if' h_yan h_S' ih =>
      ...
      have h_konf1 : KonfTipliFull Γ Δ Ρ (ifadeyleKonf S ts1 ts2 ctx b) :=
        konfTipliFull_odak Γ Δ Ρ S ts1 ts2 ctx b h_konf h_t
          ⟨Tip.scalar, Λ0, Ρ0, typed_carp_sag h_ty0⟩
          (fun z h => by rw [h_if]; exact HedefVar.carp_sag _ b z h)
          (fun bb h => by rw [h_if]; exact HedefBolge.carp_sag _ b bb h)
      obtain ⟨h_nei, h_A, h_B⟩ :=
        ih h_konf1 ts1 ts2 { ctx with ifade := b } ts2' ctx' rfl h_t1' h_yan
      have h_ne : ({ ctx' with ifade := Ifade.carp (.sabit v) b' } : ThreadCtx)
          ≠ ctx := by
        intro h_e
        have h_if2 : Ifade.carp (.sabit v) b' = ctx.ifade :=
          congrArg ThreadCtx.ifade h_e
        have h_lin2x := congrArg ThreadCtx.lineer h_e
        have h_lin2 : ctx'.lineer = ctx.lineer := h_lin2x
        rw [h_if] at h_if2
        ...
        show (⟨ctx'.tid, ctx'.ifade, ctx'.lineer⟩ : ThreadCtx)
            = ⟨ctx.tid, b, ctx.lineer⟩
        rw [h_tid, h_if', h_bb, h_lin2]
      have h2' : ts1 ++ ({ ctx' with ifade := Ifade.carp (.sabit v) b' } : ThreadCtx)
          :: ts2' = ts1o ++ ctxo' :: ts2o' := h2
      obtain ⟨h_e1, h_e2, h_e3, h_e4, h_e5⟩ :=
        cerrah_kilit ts1o ts1 (h_t.symm.trans h1) h2' h_yan h_yano h_ne
      rw [h_e2, h_e3]
      ...
      · intro τo Λo Ρoo h_ty
        ...
        | HasType.t_carp _ _ _ _ hta htb,
          ...
          rw [h_if'] at h_tyb'
          exact ⟨Λb', Ρb',
            ⟨HasType.t_carp _ _ _ _ hta h_tyb'.hasType,
             LineerTamam.l_carp _ _ _ _ (.sabit v) b'
               (LineerTamam.l_sabit _ _ v) h_tyb'.lineerOK,
             RegionTamam.r_carp _ _ _ _ (.sabit v) b'
               (RegionTamam.r_sabit _ _ v) h_tyb'.regionOK⟩,
            h_kb, h_ib⟩
      · exact h_B
  -- D-337: indeksli yazma cong'lari — sAtamaCong/sToplaCong deseni.
  | sIndeksAtaCongIdx S S' S1 S1' ts1 ts2 ts2' ctx ctx' x idx idx' e h_t h_if h_S1 h_inner h_t1' h_tid h_if' h_yan h_S' ih =>
      intro h_konf ts1o ts2o ctxo ts2o' ctxo' h1 h2 h_yano
      subst h_S1
      subst h_S'
      ...
      obtain ⟨τ0, Λ0, Ρ0, h_ty0⟩ := h_konf.2.1 ctx h_ctx_in
      rw [h_if] at h_ty0
      have h_konf1 : KonfTipliFull Γ Δ Ρ (ifadeyleKonf S ts1 ts2 ctx idx) :=
        konfTipliFull_odak Γ Δ Ρ S ts1 ts2 ctx idx h_konf h_t
          (typed_indeks_ata_idx h_ty0)
          (fun z h => by rw [h_if]; exact HedefVar.indeksAta_idx x idx e z h)
          (fun bb h => by rw [h_if]; exact HedefBolge.indeksAta_idx x idx e bb h)
      obtain ⟨h_nei, h_A, h_B⟩ :=
        ih h_konf1 ts1 ts2 { ctx with ifade := idx } ts2' ctx' rfl h_t1' h_yan
      have h_ne : ({ ctx' with ifade := Ifade.indeksAta x idx' e } : ThreadCtx)
          ≠ ctx := by
        intro h_e
        have h_if2 : Ifade.indeksAta x idx' e = ctx.ifade :=
          congrArg ThreadCtx.ifade h_e
        have h_lin2x := congrArg ThreadCtx.lineer h_e
        have h_lin2 : ctx'.lineer = ctx.lineer := h_lin2x
        rw [h_if] at h_if2
        injection h_if2 with _ h_ii _
        apply h_nei
        show (⟨ctx'.tid, ctx'.ifade, ctx'.lineer⟩ : ThreadCtx)
            = ⟨ctx.tid, idx, ctx.lineer⟩
        rw [h_tid, h_if', h_ii, h_lin2]
      have h2' : ts1 ++ ({ ctx' with ifade := Ifade.indeksAta x idx' e } : ThreadCtx)
          :: ts2' = ts1o ++ ctxo' :: ts2o' := h2
      obtain ⟨h_e1, h_e2, h_e3, h_e4, h_e5⟩ :=
        cerrah_kilit ts1o ts1 (h_t.symm.trans h1) h2' h_yan h_yano h_ne
      rw [h_e2, h_e3]
      ...
      · intro τo Λo Ρoo h_ty
        ...
        | HasType.t_indeks_ata _ _ _ _ _ h_gx hti hte,
          LineerTamam.l_indeks_ata _ _ Λi _ _ _ _ τx2 h_gx2 h_lin2 hli hle,
          RegionTamam.r_indeks_ata _ _ Ρi _ _ _ _ bA hri hre h_gxA h_yzA =>
          obtain ⟨Λi', Ρi', h_tyi', h_ki, h_ii⟩ :=
            h_A Tip.scalar Λi Ρi ⟨hti, hli, hri⟩
          rw [h_if'] at h_tyi'
          obtain ⟨Λe', h_le', h_ke⟩ := lineerTamam_kucuk_transport hle Λi' h_ki
          obtain ⟨Ρe', h_re', h_ie⟩ := regionTamam_iliski_transport hre Ρi' h_ii
          exact ⟨Λe', Ρe',
            ⟨HasType.t_indeks_ata _ _ _ _ _ h_gx h_tyi'.hasType hte,
             LineerTamam.l_indeks_ata _ _ _ _ x idx' e τx2 h_gx2 h_lin2
               h_tyi'.lineerOK h_le',
             RegionTamam.r_indeks_ata _ _ _ _ x idx' e bA
               h_tyi'.regionOK h_re' (h_ie.1 x bA h_gxA h_yzA) h_yzA⟩,
            h_ke, h_ie⟩
      · exact h_B
  | sIndeksAtaCongDeg S S' S1 S1' ts1 ts2 ts2' ctx ctx' x v e e' h_t h_if h_S1 h_inner h_t1' h_tid h_if' h_yan h_S' ih =>
      intro h_konf ts1o ts2o ctxo ts2o' ctxo' h1 h2 h_yano
      ...
      have h_konf1 : KonfTipliFull Γ Δ Ρ (ifadeyleKonf S ts1 ts2 ctx e) :=
        konfTipliFull_odak Γ Δ Ρ S ts1 ts2 ctx e h_konf h_t
          ⟨Tip.scalar, Λ0, Ρ0, typed_indeks_ata_deg h_ty0⟩
          (fun z h => by rw [h_if]; exact HedefVar.indeksAta_deg x _ e z h)
          (fun bb h => by rw [h_if]; exact HedefBolge.indeksAta_deg x _ e bb h)
      obtain ⟨h_nei, h_A, h_B⟩ :=
        ih h_konf1 ts1 ts2 { ctx with ifade := e } ts2' ctx' rfl h_t1' h_yan
      have h_ne : ({ ctx' with ifade := Ifade.indeksAta x (.sabit v) e' } : ThreadCtx)
          ≠ ctx := by
        intro h_e
        have h_if2 : Ifade.indeksAta x (.sabit v) e' = ctx.ifade :=
          congrArg ThreadCtx.ifade h_e
        have h_lin2x := congrArg ThreadCtx.lineer h_e
        have h_lin2 : ctx'.lineer = ctx.lineer := h_lin2x
        rw [h_if] at h_if2
        injection h_if2 with _ _ h_ee
        apply h_nei
        show (⟨ctx'.tid, ctx'.ifade, ctx'.lineer⟩ : ThreadCtx)
            = ⟨ctx.tid, e, ctx.lineer⟩
        rw [h_tid, h_if', h_ee, h_lin2]
      have h2' : ts1 ++ ({ ctx' with ifade := Ifade.indeksAta x (.sabit v) e' }
          : ThreadCtx) :: ts2' = ts1o ++ ctxo' :: ts2o' := h2
      obtain ⟨h_e1, h_e2, h_e3, h_e4, h_e5⟩ :=
        cerrah_kilit ts1o ts1 (h_t.symm.trans h1) h2' h_yan h_yano h_ne
      rw [h_e2, h_e3]
      ...
      · intro τo Λo Ρoo h_ty
        ...
        match ht, hl, hr with
        | HasType.t_indeks_ata _ _ _ _ _ h_gx hti hte,
          LineerTamam.l_indeks_ata _ _ _ _ _ _ _ τx2 h_gx2 h_lin2 hli hle,
          RegionTamam.r_indeks_ata _ _ _ _ _ _ _ bA hri hre h_gxA h_yzA =>
          cases hli; cases hri
          obtain ⟨Λe', Ρe', h_tye', h_ke, h_ie⟩ :=
            h_A Tip.scalar Λo Ρoo ⟨hte, hle, hre⟩
          rw [h_if'] at h_tye'
          exact ⟨Λe', Ρe',
            ⟨HasType.t_indeks_ata _ _ _ _ _ h_gx hti h_tye'.hasType,
             LineerTamam.l_indeks_ata _ _ _ _ x (.sabit v) e' τx2 h_gx2 h_lin2
               (LineerTamam.l_sabit _ _ v) h_tye'.lineerOK,
             RegionTamam.r_indeks_ata _ _ _ _ x (.sabit v) e' bA
               (RegionTamam.r_sabit _ _ v) h_tye'.regionOK
               (h_ie.1 x bA h_gxA h_yzA) h_yzA⟩,
            h_ke, h_ie⟩
      · exact h_B
  ...
  | sEslesCong S S' S1 S1' ts1 ts2 ts2' ctx ctx' s s' n d y h_t h_if h_S1 h_inner h_t1' h_tid h_if' h_yan h_S' ih =>
      ...
      obtain ⟨τ0, Λ0, Ρ0, h_ty0⟩ := h_konf.2.1 ctx h_ctx_in
      rw [h_if] at h_ty0
      have h_konf1 : KonfTipliFull Γ Δ Ρ (ifadeyleKonf S ts1 ts2 ctx s) :=
        konfTipliFull_odak Γ Δ Ρ S ts1 ts2 ctx s h_konf h_t
          (typed_esles_skrut h_ty0)
          (fun z h => by rw [h_if]; exact HedefVar.esles_skrut s n d y z h)
          (fun bb h => by rw [h_if]; exact HedefBolge.esles_skrut s n d y bb h)
      obtain ⟨h_nei, h_A, h_B⟩ :=
        ih h_konf1 ts1 ts2 { ctx with ifade := s } ts2' ctx' rfl h_t1' h_yan
      have h_ne : ({ ctx' with ifade := Ifade.esles s' n d y } : ThreadCtx)
          ≠ ctx := by
        intro h_e
        have h_if2 : Ifade.esles s' n d y = ctx.ifade := congrArg ThreadCtx.ifade h_e
        have h_lin2x := congrArg ThreadCtx.lineer h_e
        have h_lin2 : ctx'.lineer = ctx.lineer := h_lin2x
        rw [h_if] at h_if2
        injection h_if2 with h_ss _ _ _
        apply h_nei
        show (⟨ctx'.tid, ctx'.ifade, ctx'.lineer⟩ : ThreadCtx)
            = ⟨ctx.tid, s, ctx.lineer⟩
        rw [h_tid, h_if', h_ss, h_lin2]
      have h2' : ts1 ++ ({ ctx' with ifade := Ifade.esles s' n d y } : ThreadCtx)
          :: ts2' = ts1o ++ ctxo' :: ts2o' := h2
      obtain ⟨h_e1, h_e2, h_e3, h_e4, h_e5⟩ :=
        cerrah_kilit ts1o ts1 (h_t.symm.trans h1) h2' h_yan h_yano h_ne
      rw [h_e2, h_e3]
      ...
      · intro τo Λo Ρoo h_ty
        ...
        obtain ⟨ht, hl, hr⟩ := h_ty
        match ht, hl, hr with
        | HasType.t_esles _ _ _ _ _ _ τ hts htd hty,
          LineerTamam.l_esles _ _ Λs _ _ _ _ hls h_nd h_ny,
          RegionTamam.r_esles _ _ Ρs _ _ _ _ hrs h_rnd h_rny hrd hry =>
          obtain ⟨Λs', Ρs', h_tys', h_ks, h_is⟩ :=
            h_A Tip.scalar Λs Ρs ⟨hts, hls, hrs⟩
          rw [h_if'] at h_tys'
          obtain ⟨Ρdn, h_rd', _⟩ := regionTamam_iliski_transport hrd Ρs' h_is
          obtain ⟨Ρyn, h_ry', _⟩ := regionTamam_iliski_transport hry Ρs' h_is
          rw [regionNotr_cikis_esit h_rd' h_rnd] at h_rd'
          rw [regionNotr_cikis_esit h_ry' h_rny] at h_ry'
          exact ⟨Λs', Ρs',
            ⟨HasType.t_esles _ _ _ _ _ _ τ h_tys'.hasType htd hty,
             LineerTamam.l_esles _ _ _ s' n d y h_tys'.lineerOK h_nd h_ny,
             RegionTamam.r_esles _ _ _ s' n d y h_tys'.regionOK h_rnd h_rny
               h_rd' h_ry'⟩,
            h_ks, h_is⟩
      · exact h_B
  | sGuvensizCong S S' S1 S1' ts1 ts2 ts2' ctx ctx' e e' h_t h_if h_S1 h_inner h_t1' h_tid h_if' h_yan h_S' ih =>
...
/-- BIRLESIK ADIM KORUNUMU (FAZ_BRIFINGLERI.md F4 madde 2):
    ...
    - Congruence (3): TAM — adim_odak_yuku (odak-adim yuku) +
      lineerTamam_kucuk_transport + regionTamam_iliski_transport
      kompozisyonu (FIX-F cerceve yan-kosulu + FIX-G yazma-ani
      kontrolu sayesinde; DECISIONS_LOG). **21/21 — SORRY 0.** -/
theorem adim_korunum
  ...
  | sSeqAtla S S' ts1 ts2 ctx v b h_t h_if h_S' =>
      ...
      · have h13 := h_konf.2.2.2.2.2.2.2.2.2.2.2.2.1
        rw [h_t] at h13
        exact tidAyrik_degisim h13 _ rfl
  -- D-332: sEgerSec — sSeqAtla ile ayni sekil. Secilen dal odaga girer;
  -- bolge/sahiplik/kanal DEGISMEZ (yalniz iz'e dalOl eklenir, ki
  -- KonfTipliFull iz'i kisitlamaz).
  | sEgerSec S S' ts1 ts2 ctx v d y alindi h_t h_if h_dal h_S' =>
      intro h_konf
      subst h_S'
      have h_ctx_in : ctx ∈ S.thread := by
      ...
      refine ⟨Ρ, h_konf.1, ?_, h_konf.2.2.1, h_konf.2.2.2.1, rfl,
              ...
              h_konf.2.2.2.2.2.2.2.2.2.2.2.2.2.1,
              h_konf.2.2.2.2.2.2.2.2.2.2.2.2.2.2⟩
      · -- comp 2: eger (sabit v) d y → secilen dal (typed_eger_dal)
        have h_thread := h_konf.2.1
        obtain ⟨τ0, Λ0, Ρ0, h_ty⟩ := h_thread ctx h_ctx_in
        rw [h_if] at h_ty
        have h_thread' := h_thread
        ...
        exact threadTipli_degisim h_thread' _
          ⟨τ0, Λ0, Ρ0, typed_eger_dal h_ty alindi⟩
      · -- comp 8: dalin hedefleri eger_dogru/eger_yanlis ile kapsaniyordu
        have h8 := h_konf.2.2.2.2.2.2.2.1
        rw [h_t] at h8
        refine hedefVarSahip_degisim h8 _ rfl (fun z h => ?_)
        rw [h_if]
        cases alindi with
        | true  => exact HedefVar.eger_dogru _ d y z (by simpa using h)
        | false => exact HedefVar.eger_yanlis _ d y z (by simpa using h)
      · -- comp 9
        have h9 := h_konf.2.2.2.2.2.2.2.2.1
        rw [h_t] at h9
        refine hedefBolgeSahip_degisim h9 _ rfl (fun bb h => ?_)
        rw [h_if]
        cases alindi with
        | true  => exact HedefBolge.eger_dogru _ d y bb (by simpa using h)
        | false => exact HedefBolge.eger_yanlis _ d y bb (by simpa using h)
      · have h13 := h_konf.2.2.2.2.2.2.2.2.2.2.2.2.1
        rw [h_t] at h13
        exact tidAyrik_degisim h13 _ rfl
  -- D-338: sBolTamam — sToplaTamam sekli (olay uretir ama KonfTipliFull
  -- izi kisitlamaz; store/sahiplik/bolge degismez).
  | sBolTamam S S' ts1 ts2 ctx n1 n2 h_t h_if h_S' =>
      intro h_konf
      subst h_S'
      have h_ctx_in : ctx ∈ S.thread := by
      ...
      · have h_thread := h_konf.2.1
        ...
        have h_thread' := h_thread
        rw [h_t] at h_thread'
        exact threadTipli_degisim h_thread' _ ⟨τ0, Λ0, Ρ0, typed_bol_atla h_ty⟩
      · have h8 := h_konf.2.2.2.2.2.2.2.1
        rw [h_t] at h8
        exact hedefVarSahip_degisim h8 _ rfl (fun y h => nomatch h)
      · have h9 := h_konf.2.2.2.2.2.2.2.2.1
  ...
  | sKalanTamam S S' ts1 ts2 ctx n1 n2 h_t h_if h_S' =>
      ...
      · have h_thread := h_konf.2.1
        ...
        have h_thread' := h_thread
        rw [h_t] at h_thread'
        exact threadTipli_degisim h_thread' _ ⟨τ0, Λ0, Ρ0, typed_kalan_atla h_ty⟩
      · have h8 := h_konf.2.2.2.2.2.2.2.1
        rw [h_t] at h8
        exact hedefVarSahip_degisim h8 _ rfl (fun y h => nomatch h)
      · have h9 := h_konf.2.2.2.2.2.2.2.2.1
      ...
      · have h13 := h_konf.2.2.2.2.2.2.2.2.2.2.2.2.1
        rw [h_t] at h13
        exact tidAyrik_degisim h13 _ rfl
  -- D-334: sToplaTamam — sSeqAtla sekli; hedefi olmayan bir deger uretir.
  | sToplaTamam S S' ts1 ts2 ctx n1 n2 h_t h_if h_S' =>
      intro h_konf
      subst h_S'
      have h_ctx_in : ctx ∈ S.thread := by
      ...
      · have h_thread := h_konf.2.1
        ...
        have h_thread' := h_thread
        rw [h_t] at h_thread'
        exact threadTipli_degisim h_thread' _ ⟨τ0, Λ0, Ρ0, typed_topla_atla h_ty⟩
      · have h8 := h_konf.2.2.2.2.2.2.2.1
        rw [h_t] at h8
        exact hedefVarSahip_degisim h8 _ rfl (fun y h => nomatch h)
      · have h9 := h_konf.2.2.2.2.2.2.2.2.1
  ...
  | sCarpTamam S S' ts1 ts2 ctx n1 n2 h_t h_if h_S' =>
      ...
      · have h_thread := h_konf.2.1
        ...
        have h_thread' := h_thread
        rw [h_t] at h_thread'
        exact threadTipli_degisim h_thread' _ ⟨τ0, Λ0, Ρ0, typed_carp_atla h_ty⟩
      · have h8 := h_konf.2.2.2.2.2.2.2.1
        rw [h_t] at h8
        exact hedefVarSahip_degisim h8 _ rfl (fun y h => nomatch h)
      · have h9 := h_konf.2.2.2.2.2.2.2.2.1
      ...
      · have h13 := h_konf.2.2.2.2.2.2.2.2.2.2.2.2.1
        rw [h_t] at h13
        exact tidAyrik_degisim h13 _ rfl
  -- D-335: sIkenAc — acilma. Ortamlar sabit; hedefler `hedef*_iken_ac`
  -- ile orijinal donguye geri eslenir (invaryant korunur).
  | sIkenAc S S' ts1 ts2 ctx k g h_t h_if h_S' =>
      intro h_konf
      subst h_S'
      have h_ctx_in : ctx ∈ S.thread := by
      ...
      · have h_thread := h_konf.2.1
        ...
        have h_thread' := h_thread
        rw [h_t] at h_thread'
        exact threadTipli_degisim h_thread' _ ⟨τ0, Λ0, Ρ0, typed_iken_ac h_ty⟩
      · have h8 := h_konf.2.2.2.2.2.2.2.1
        rw [h_t] at h8
        exact hedefVarSahip_degisim h8 _ rfl
          (fun z h => by rw [h_if]; exact hedefVar_iken_ac h)
      · have h9 := h_konf.2.2.2.2.2.2.2.2.1
        rw [h_t] at h9
        exact hedefBolgeSahip_degisim h9 _ rfl
      ...
      · have h13 := h_konf.2.2.2.2.2.2.2.2.2.2.2.2.1
        rw [h_t] at h13
        exact tidAyrik_degisim h13 _ rfl
  -- D-337: sIndeksYaz — sAtamaTamam sekli, ama yazma OFSET i'ye.
  -- `bagli` bileseni ofset-0'i konusur: i ≠ 0 ise golgeleme YOKTUR
  -- (kolay kol); i = 0 ise sAtamaTamam'in BolgeAyrik argumani aynen
  -- gecerlidir. `t_indeks_ata`nin `Γ x = scalar` sarti tam burada is
  -- goruyor: yazilan `skaler n`in tipi hedefin tipiyle uyusmali.
  | sIndeksYaz S S' ts1 ts2 ctx x i n b h_t h_if h_b h_owner h_S' =>
      intro h_konf
      subst h_S'
      have h_ctx_in : ctx ∈ S.thread := by
      ...
      rw [h_if] at h_ty
      have h_gx : tipOrtamGet Γ x = some Tip.scalar := by
        match h_ty.hasType with
        | HasType.t_indeks_ata _ _ _ _ _ h_g _ _ => exact h_g
      ...
      refine ⟨Ρ, ?_, ?_, h_konf.2.2.1, h_konf.2.2.2.1, rfl,
              ...
              h_konf.2.2.2.2.2.2.2.2.2.2.2.2.2.1,
              h_konf.2.2.2.2.2.2.2.2.2.2.2.2.2.2⟩
      · -- comp 1: yeni store girisi tipli (scalar) + bolgesi kayitli
        intro kk ww h_kv
        rcases List.mem_cons.mp h_kv with h_head | h_tail
        · obtain ⟨h_kk, h_ww⟩ := Prod.mk.injEq .. ▸ h_head
          refine ⟨Tip.scalar, ?_, ⟨x, b, ?_, ?_⟩⟩
          · rw [h_ww]; exact DegerTipli.dt_skaler n
          · rw [← h_beq]; exact h_b
          · rw [h_kk]
        · exact h_konf.1 kk ww h_tail
      · -- comp 2: odakli thread `birim`e ilerledi
        have h_thread := h_konf.2.1
        rw [h_t] at h_thread
        exact threadTipli_degisim h_thread _
      ...
      · -- comp 9
        ...
        rw [h_t] at h9
        exact hedefBolgeSahip_degisim h9 _ rfl (fun b' h => nomatch h)
      · -- comp 10: store-golgeleme — ofset ayrimi
        intro z τz h_gz
        obtain ⟨bz, vz, h_bz, h_kz, h_dtz⟩ :=
          h_konf.2.2.2.2.2.2.2.2.2.1 z τz h_gz
        by_cases h_of : i.toNat = 0
        · -- ofset 0: sAtamaTamam ile ayni argument
          by_cases h_eqk : b.id = bz.id
          · have h_zx : z = x := by
              have h_ba := h_konf.2.2.2.2.2.2.2.2.2.2.2.1
              exact h_ba z x bz b h_bz h_b h_eqk.symm
            subst h_zx
            have h_tau : τz = Tip.scalar := Option.some.inj (h_gz.symm.trans h_gx)
            refine ⟨bz, .skaler n, h_bz, ?_, ?_⟩
            · show konumGet ((⟨b, i.toNat⟩, Deger.skaler n) :: S.store) ⟨bz, 0⟩
                  = some (.skaler n)
              rw [konumGet, if_pos (⟨h_eqk, h_of⟩ : _ ∧ _)]
            · rw [h_tau]; exact DegerTipli.dt_skaler n
          · refine ⟨bz, vz, h_bz, ?_, h_dtz⟩
            show konumGet ((⟨b, i.toNat⟩, Deger.skaler n) :: S.store) ⟨bz, 0⟩
                = some vz
            rw [konumGet, if_neg (fun hc => h_eqk hc.1)]
            exact h_kz
        · -- ofset ≠ 0: golgeleme YOK
          refine ⟨bz, vz, h_bz, ?_, h_dtz⟩
          show konumGet ((⟨b, i.toNat⟩, Deger.skaler n) :: S.store) ⟨bz, 0⟩
              = some vz
          rw [konumGet, if_neg (fun hc => h_of hc.2)]
          exact h_kz
      · -- comp 13
        have h13 := h_konf.2.2.2.2.2.2.2.2.2.2.2.2.1
        rw [h_t] at h13
  ...
  | sEslesSec S S' ts1 ts2 ctx m n d y tuttu h_t h_if h_dal h_S' =>
      ...
      · have h_thread := h_konf.2.1
        ...
        rw [h_t] at h_thread'
        exact threadTipli_degisim h_thread' _
          ⟨τ0, Λ0, Ρ0, typed_esles_dal h_ty tuttu⟩
      · have h8 := h_konf.2.2.2.2.2.2.2.1
        rw [h_t] at h8
        refine hedefVarSahip_degisim h8 _ rfl (fun z h => ?_)
        rw [h_if]
        cases tuttu with
        | true  => exact HedefVar.esles_eslesen _ n d y z (by simpa using h)
        | false => exact HedefVar.esles_kalan _ n d y z (by simpa using h)
      · have h9 := h_konf.2.2.2.2.2.2.2.2.1
        rw [h_t] at h9
        refine hedefBolgeSahip_degisim h9 _ rfl (fun bb h => ?_)
        rw [h_if]
        cases tuttu with
        | true  => exact HedefBolge.esles_eslesen _ n d y bb (by simpa using h)
        | false => exact HedefBolge.esles_kalan _ n d y bb (by simpa using h)
      · have h13 := h_konf.2.2.2.2.2.2.2.2.2.2.2.2.1
        rw [h_t] at h13
        exact tidAyrik_degisim h13 _ rfl
  | sGuvensizAtla S S' ts1 ts2 ctx v h_t h_if h_S' =>
  ...
  | sLinImhaHataZatenTuketildi S S' ts1 ts2 ctx x h_t h_if h_tuket h_S' =>
      ...
      exact typing_excludes_sLinImhaHataZatenTuketildi Γ Δ ctx.lineer Ρ
        x τ' Λ'' Ρ'' h_typed h_tuket
  -- ============ Congruence — odak-yuku kompozisyonu (TAM) ============
  -- adim_odak_yuku ic-adimin odakli ciftine (E)/(A)/(B) verir; devam-
  -- ifadesi lineerTamam_kucuk_transport + regionTamam_iliski_transport
  -- ile yeniden tiplenir; buyuyen hedef-kumesi (B) + on-durum comp-8/9
  -- ile tasinir. Diger 12 bilesen S1'-gecisinden aynen.
  | sSeqCong S S' S1 S1' ts1 ts2 ts2' ctx ctx' a a' b h_t h_if h_S1 h_inner h_t1' h_tid h_if' h_yan h_S' ih =>
      intro h_konf
      subst h_S1
      have h_ctx_in : ctx ∈ S.thread := by
      ...
      obtain ⟨h_nei, h_A, h_B⟩ :=
        adim_odak_yuku Γ Δ Ρ _ S1' h_inner h_konf1
          ts1 ts2 { ctx with ifade := a } ts2' ctx' rfl h_t1' h_yan
      subst h_S'
        ...
        · rcases List.mem_cons.mp h2 with he | h3
          · subst he
            ...
            | HasType.t_seq _ _ _ _ τa _ hta htb,
              ...
              rw [← h_beq] at hra
              obtain ⟨Λa', Ρa', h_tya', h_ka, h_ia⟩ :=
                h_A τa Λa Ρa ⟨hta, hla, hra⟩
              rw [h_if'] at h_tya'
              obtain ⟨Λb', h_lb', _⟩ := lineerTamam_kucuk_transport hlb Λa' h_ka
              obtain ⟨Ρb', h_rb', _⟩ := regionTamam_iliski_transport hrb Ρa' h_ia
              rw [h_beq1] at h_tya'
              exact ⟨τ0, Λb', Ρb',
                ⟨HasType.t_seq _ _ _ _ _ _ h_tya'.hasType htb,
                 LineerTamam.l_seq _ _ _ _ a' b h_tya'.lineerOK h_lb',
      ...
      · -- comp 8
        ...
        · exact h_konf1'.2.2.2.2.2.2.2.1 c
            (by rw [h_t1']; exact List.mem_append.mpr (Or.inl h1)) y hy bb h_bb h_yz
        · rcases List.mem_cons.mp h2 with he | h3
          · subst he
            have hy2 : HedefVar (Ifade.seq a' b) y := hy
            have h_bb2 : bolgeOrtamGet S1'.bolge y = some bb := h_bb
            rcases hedefVar_seq_inv hy2 with h_a' | h_b
            · have h_ctx'_in : ctx' ∈ S1'.thread := by
                rw [h_t1']
                exact List.mem_append.mpr (Or.inr (List.Mem.head _))
              exact h_konf1'.2.2.2.2.2.2.2.1 ctx' h_ctx'_in y
                (by rw [h_if']; exact h_a') bb h_bb2 h_yz
            · obtain ⟨h_pre, h_pres⟩ := h_B y bb h_bb2 h_yz
              have h_pre2 : bolgeOrtamGet S.bolge y = some bb := h_pre
              have h_own0 := h_konf.2.2.2.2.2.2.2.1 ctx h_ctx_in y
                (by rw [h_if]; exact HedefVar.seq_sag a b y h_b) bb h_pre2 h_yz
              have h_own1 := h_pres h_own0
              have h_tid2 : ({ ctx' with ifade := Ifade.seq a' b }
                  : ThreadCtx).tid = ctx.tid := h_tid
              show sahiplikGet S1'.sahiplik bb = some (Sahip.thread
                ({ ctx' with ifade := Ifade.seq a' b } : ThreadCtx).tid)
              rw [h_tid2]
              exact h_own1
          · exact h_konf1'.2.2.2.2.2.2.2.1 c
              (by rw [h_t1']
      ...
      · -- comp 9
        ...
        · rcases List.mem_cons.mp h2 with he | h3
          · subst he
            have hb2 : HedefBolge (Ifade.seq a' b) bb := hb
            rcases hedefBolge_seq_inv hb2 with h_a' | h_b
            · have h_ctx'_in : ctx' ∈ S1'.thread := by
                rw [h_t1']
                exact List.mem_append.mpr (Or.inr (List.Mem.head _))
              exact h_konf1'.2.2.2.2.2.2.2.2.1 ctx' h_ctx'_in bb
                (by rw [h_if']; exact h_a') h_kayit h_yz
            · obtain ⟨x0, h_x0⟩ := h_kayit
              have h_x02 : bolgeOrtamGet S1'.bolge x0 = some bb := h_x0
              obtain ⟨h_prex, h_pres⟩ := h_B x0 bb h_x02 h_yz
              have h_prex2 : bolgeOrtamGet S.bolge x0 = some bb := h_prex
              have h_own0 := h_konf.2.2.2.2.2.2.2.2.1 ctx h_ctx_in bb
                (by rw [h_if]; exact HedefBolge.seq_sag a b bb h_b)
                ⟨x0, h_prex2⟩ h_yz
              have h_own1 := h_pres h_own0
              have h_tid2 : ({ ctx' with ifade := Ifade.seq a' b }
                  : ThreadCtx).tid = ctx.tid := h_tid
      ...
      · -- comp 13
        ...
        rw [h_t1'] at h13
        exact tidAyrik_degisim h13 _ rfl
  -- D-332: sEgerCong — sSeqCong ile ayni iskelet. Dallar icin ortam
  -- tasimasi gerekmez (LineerNotr/RegionNotr sozdizimsel); r_eger'in
  -- Ρ-bagli dal premise'leri iliski-transport + notr-kimlik ile yeniden
  -- kurulur. HedefVar/HedefBolge yukumlulukleri: kosul kolu ctx''den,
  -- dal kollari h_B (odak-yuku onceki-ortam kanadi) uzerinden.
  | sEgerCong S S' S1 S1' ts1 ts2 ts2' ctx ctx' k k' d y h_t h_if h_S1 h_inner h_t1' h_tid h_if' h_yan h_S' ih =>
      intro h_konf
      subst h_S1
      have h_ctx_in : ctx ∈ S.thread := by
      ...
      obtain ⟨h_nei, h_A, h_B⟩ :=
        adim_odak_yuku Γ Δ Ρ _ S1' h_inner h_konf1
          ts1 ts2 { ctx with ifade := k } ts2' ctx' rfl h_t1' h_yan
      subst h_S'
      have h_beq : S.bolge = Ρ := h_konf.2.2.2.2.2.1
      have h_beq1 : S1'.bolge = Ρ1' := h_konf1'.2.2.2.2.2.1
      refine ⟨Ρ1', h_konf1'.1, ?_, h_konf1'.2.2.1, h_konf1'.2.2.2.1,
        ...
        · rcases List.mem_cons.mp h2 with he | h3
          · subst he
            ...
            | HasType.t_eger _ _ _ _ _ τk _ htk htd hty,
              ...
              RegionTamam.r_eger _ _ Ρk _ _ _ hrk h_rnd h_rny hrd hry =>
              rw [← h_beq] at hrk
              obtain ⟨Λk', Ρk', h_tyk', h_kk, h_ik⟩ :=
                h_A τk Λk Ρk ⟨htk, hlk, hrk⟩
              rw [h_if'] at h_tyk'
              obtain ⟨Ρdn, h_rd', _⟩ := regionTamam_iliski_transport hrd Ρk' h_ik
              obtain ⟨Ρyn, h_ry', _⟩ := regionTamam_iliski_transport hry Ρk' h_ik
              rw [regionNotr_cikis_esit h_rd' h_rnd] at h_rd'
      ...
      · -- comp 8
        ...
        · rcases List.mem_cons.mp h2 with he | h3
          · subst he
            have hz2 : HedefVar (Ifade.eger k' d y) z := hz
            have h_bb2 : bolgeOrtamGet S1'.bolge z = some bb := h_bb
            have h_ctx'_in : ctx' ∈ S1'.thread := by
              rw [h_t1']
              exact List.mem_append.mpr (Or.inr (List.Mem.head _))
            have h_dal : ∀ (hz3 : HedefVar (Ifade.eger k d y) z),
                sahiplikGet S1'.sahiplik bb = some (Sahip.thread
                  ({ ctx' with ifade := Ifade.eger k' d y } : ThreadCtx).tid) := by
              intro hz3
              obtain ⟨h_pre, h_pres⟩ := h_B z bb h_bb2 h_yz
              have h_pre2 : bolgeOrtamGet S.bolge z = some bb := h_pre
              have h_own0 := h_konf.2.2.2.2.2.2.2.1 ctx h_ctx_in z
                (by rw [h_if]; exact hz3) bb h_pre2 h_yz
              have h_tid2 : ({ ctx' with ifade := Ifade.eger k' d y }
                  : ThreadCtx).tid = ctx.tid := h_tid
              rw [h_tid2]
              exact h_pres h_own0
            cases hz2 with
            | eger_kosul _ _ _ _ h_k' =>
                exact h_konf1'.2.2.2.2.2.2.2.1 ctx' h_ctx'_in z
                  (by rw [h_if']; exact h_k') bb h_bb2 h_yz
            | eger_dogru _ _ _ _ h_d' =>
                exact h_dal (HedefVar.eger_dogru k d y z h_d')
            | eger_yanlis _ _ _ _ h_y' =>
                exact h_dal (HedefVar.eger_yanlis k d y z h_y')
          · exact h_konf1'.2.2.2.2.2.2.2.1 c
              (by rw [h_t1']
                  exact List.mem_append.mpr (Or.inr (List.Mem.tail _ h3)))
              z hz bb h_bb h_yz
      · -- comp 9
        ...
        · rcases List.mem_cons.mp h2 with he | h3
          · subst he
            have hb2 : HedefBolge (Ifade.eger k' d y) bb := hb
            have h_ctx'_in : ctx' ∈ S1'.thread := by
              rw [h_t1']
              exact List.mem_append.mpr (Or.inr (List.Mem.head _))
            have h_dal : ∀ (hb3 : HedefBolge (Ifade.eger k d y) bb),
                sahiplikGet S1'.sahiplik bb = some (Sahip.thread
                  ({ ctx' with ifade := Ifade.eger k' d y } : ThreadCtx).tid) := by
              intro hb3
              obtain ⟨x0, h_x0⟩ := h_kayit
              ...
              have h_prex2 : bolgeOrtamGet S.bolge x0 = some bb := h_prex
              have h_own0 := h_konf.2.2.2.2.2.2.2.2.1 ctx h_ctx_in bb
                (by rw [h_if]; exact hb3) ⟨x0, h_prex2⟩ h_yz
              have h_tid2 : ({ ctx' with ifade := Ifade.eger k' d y }
                  : ThreadCtx).tid = ctx.tid := h_tid
              rw [h_tid2]
              exact h_pres h_own0
            ...
            | eger_kosul _ _ _ _ h_k' =>
                exact h_konf1'.2.2.2.2.2.2.2.2.1 ctx' h_ctx'_in bb
                  (by rw [h_if']; exact h_k') h_kayit h_yz
            | eger_dogru _ _ _ _ h_d' =>
                exact h_dal (HedefBolge.eger_dogru k d y bb h_d')
            | eger_yanlis _ _ _ _ h_y' =>
                exact h_dal (HedefBolge.eger_yanlis k d y bb h_y')
          · exact h_konf1'.2.2.2.2.2.2.2.2.1 c
              (by rw [h_t1']
                  exact List.mem_append.mpr (Or.inr (List.Mem.tail _ h3)))
              bb hb h_kayit h_yz
      · -- comp 13
        ...
        rw [h_t1'] at h13
        exact tidAyrik_degisim h13 _ rfl
  -- D-337: indeksli yazma cong'lari. HEDEF x oldugu icin comp 8'de
  -- `indeksAta_bas` kolu da h_B uzerinden tasinir (sSeqCong deseni).
  | sIndeksAtaCongIdx S S' S1 S1' ts1 ts2 ts2' ctx ctx' x idx idx' e h_t h_if h_S1 h_inner h_t1' h_tid h_if' h_yan h_S' ih =>
      intro h_konf
      subst h_S1
      have h_ctx_in : ctx ∈ S.thread := by
      ...
      obtain ⟨h_nei, h_A, h_B⟩ :=
        adim_odak_yuku Γ Δ Ρ _ S1' h_inner h_konf1
          ts1 ts2 { ctx with ifade := idx } ts2' ctx' rfl h_t1' h_yan
      subst h_S'
      have h_beq : S.bolge = Ρ := h_konf.2.2.2.2.2.1
      have h_beq1 : S1'.bolge = Ρ1' := h_konf1'.2.2.2.2.2.1
      refine ⟨Ρ1', h_konf1'.1, ?_, h_konf1'.2.2.1, h_konf1'.2.2.2.1,
          ...
          · subst he
            ...
            | HasType.t_indeks_ata _ _ _ _ _ h_gx hti hte,
              ...
              obtain ⟨Λi', Ρi', h_tyi', h_ki, h_ii⟩ :=
                h_A Tip.scalar Λi Ρi ⟨hti, hli, hri⟩
              rw [h_if'] at h_tyi'
              obtain ⟨Λe', h_le', _⟩ := lineerTamam_kucuk_transport hle Λi' h_ki
              obtain ⟨Ρe', h_re', h_ie⟩ := regionTamam_iliski_transport hre Ρi' h_ii
              rw [h_beq1] at h_tyi'
              exact ⟨Tip.bos, Λe', Ρe',
                ⟨HasType.t_indeks_ata _ _ _ _ _ h_gx h_tyi'.hasType hte,
      ...
      · intro c h_mem z hz bb h_bb h_yz
        ...
        · rcases List.mem_cons.mp h2 with he | h3
          · subst he
            have hz2 : HedefVar (Ifade.indeksAta x idx' e) z := hz
            have h_bb2 : bolgeOrtamGet S1'.bolge z = some bb := h_bb
            have h_ctx'_in : ctx' ∈ S1'.thread := by
              rw [h_t1']; exact List.mem_append.mpr (Or.inr (List.Mem.head _))
            have h_dal : ∀ (hz3 : HedefVar (Ifade.indeksAta x idx e) z),
                sahiplikGet S1'.sahiplik bb = some (Sahip.thread
                  ({ ctx' with ifade := Ifade.indeksAta x idx' e } : ThreadCtx).tid) := by
              intro hz3
              obtain ⟨h_pre, h_pres⟩ := h_B z bb h_bb2 h_yz
              have h_pre2 : bolgeOrtamGet S.bolge z = some bb := h_pre
              have h_own0 := h_konf.2.2.2.2.2.2.2.1 ctx h_ctx_in z
                (by rw [h_if]; exact hz3) bb h_pre2 h_yz
              have h_tid2 : ({ ctx' with ifade := Ifade.indeksAta x idx' e }
                  : ThreadCtx).tid = ctx.tid := h_tid
              rw [h_tid2]
              exact h_pres h_own0
            cases hz2 with
            | indeksAta_bas _ _ _ => exact h_dal (HedefVar.indeksAta_bas x idx e)
            | indeksAta_idx _ _ _ _ h_i' =>
                exact h_konf1'.2.2.2.2.2.2.2.1 ctx' h_ctx'_in z
                  (by rw [h_if']; exact h_i') bb h_bb2 h_yz
            | indeksAta_deg _ _ _ _ h_e' =>
                exact h_dal (HedefVar.indeksAta_deg x idx e z h_e')
          · exact h_konf1'.2.2.2.2.2.2.2.1 c
              (by rw [h_t1']
                  exact List.mem_append.mpr (Or.inr (List.Mem.tail _ h3)))
              z hz bb h_bb h_yz
      · intro c h_mem bb hb h_kayit h_yz
        ...
        · rcases List.mem_cons.mp h2 with he | h3
          · subst he
            have hb2 : HedefBolge (Ifade.indeksAta x idx' e) bb := hb
            have h_ctx'_in : ctx' ∈ S1'.thread := by
              rw [h_t1']; exact List.mem_append.mpr (Or.inr (List.Mem.head _))
            have h_dal : ∀ (hb3 : HedefBolge (Ifade.indeksAta x idx e) bb),
                sahiplikGet S1'.sahiplik bb = some (Sahip.thread
                  ({ ctx' with ifade := Ifade.indeksAta x idx' e } : ThreadCtx).tid) := by
              intro hb3
              obtain ⟨x0, h_x0⟩ := h_kayit
            ...
            | indeksAta_idx _ _ _ _ h_i' =>
                exact h_konf1'.2.2.2.2.2.2.2.2.1 ctx' h_ctx'_in bb
                  (by rw [h_if']; exact h_i') h_kayit h_yz
            | indeksAta_deg _ _ _ _ h_e' =>
                exact h_dal (HedefBolge.indeksAta_deg x idx e bb h_e')
          · exact h_konf1'.2.2.2.2.2.2.2.2.1 c
              (by rw [h_t1']
                  exact List.mem_append.mpr (Or.inr (List.Mem.tail _ h3)))
              bb hb h_kayit h_yz
  ...
  | sIndeksAtaCongDeg S S' S1 S1' ts1 ts2 ts2' ctx ctx' x v e e' h_t h_if h_S1 h_inner h_t1' h_tid h_if' h_yan h_S' ih =>
      ...
      · intro c h_mem z hz bb h_bb h_yz
        ...
        · rcases List.mem_cons.mp h2 with he | h3
          · subst he
            have hz2 : HedefVar (Ifade.indeksAta x (.sabit v) e') z := hz
            have h_bb2 : bolgeOrtamGet S1'.bolge z = some bb := h_bb
            have h_ctx'_in : ctx' ∈ S1'.thread := by
              rw [h_t1']; exact List.mem_append.mpr (Or.inr (List.Mem.head _))
            have h_dal : ∀ (hz3 : HedefVar (Ifade.indeksAta x (.sabit v) e) z),
                sahiplikGet S1'.sahiplik bb = some (Sahip.thread
                  ({ ctx' with ifade := Ifade.indeksAta x (.sabit v) e' }
                    : ThreadCtx).tid) := by
              intro hz3
              obtain ⟨h_pre, h_pres⟩ := h_B z bb h_bb2 h_yz
              have h_pre2 : bolgeOrtamGet S.bolge z = some bb := h_pre
              have h_own0 := h_konf.2.2.2.2.2.2.2.1 ctx h_ctx_in z
                (by rw [h_if]; exact hz3) bb h_pre2 h_yz
              have h_tid2 : ({ ctx' with ifade := Ifade.indeksAta x (.sabit v) e' }
                  : ThreadCtx).tid = ctx.tid := h_tid
              rw [h_tid2]
              exact h_pres h_own0
            cases hz2 with
            | indeksAta_bas _ _ _ =>
                exact h_dal (HedefVar.indeksAta_bas x (.sabit v) e)
            | indeksAta_idx _ _ _ _ h_i' => nomatch h_i'
            | indeksAta_deg _ _ _ _ h_e' =>
                exact h_konf1'.2.2.2.2.2.2.2.1 ctx' h_ctx'_in z
      ...
      · intro c h_mem bb hb h_kayit h_yz
        ...
        · rcases List.mem_cons.mp h2 with he | h3
          · subst he
            have hb2 : HedefBolge (Ifade.indeksAta x (.sabit v) e') bb := hb
            have h_ctx'_in : ctx' ∈ S1'.thread := by
              rw [h_t1']; exact List.mem_append.mpr (Or.inr (List.Mem.head _))
            cases hb2 with
            | indeksAta_idx _ _ _ _ h_i' => nomatch h_i'
  ...
  | sEslesCong S S' S1 S1' ts1 ts2 ts2' ctx ctx' s s' n d y h_t h_if h_S1 h_inner h_t1' h_tid h_if' h_yan h_S' ih =>
      ...
      obtain ⟨h_nei, h_A, h_B⟩ :=
        adim_odak_yuku Γ Δ Ρ _ S1' h_inner h_konf1
          ts1 ts2 { ctx with ifade := s } ts2' ctx' rfl h_t1' h_yan
      subst h_S'
      have h_beq : S.bolge = Ρ := h_konf.2.2.2.2.2.1
      have h_beq1 : S1'.bolge = Ρ1' := h_konf1'.2.2.2.2.2.1
      refine ⟨Ρ1', h_konf1'.1, ?_, h_konf1'.2.2.1, h_konf1'.2.2.2.1,
      ...
      · intro c h_mem z hz bb h_bb h_yz
        ...
        · rcases List.mem_cons.mp h2 with he | h3
          · subst he
            have hz2 : HedefVar (Ifade.esles s' n d y) z := hz
            have h_bb2 : bolgeOrtamGet S1'.bolge z = some bb := h_bb
            have h_ctx'_in : ctx' ∈ S1'.thread := by
              rw [h_t1']; exact List.mem_append.mpr (Or.inr (List.Mem.head _))
            have h_dal : ∀ (hz3 : HedefVar (Ifade.esles s n d y) z),
                sahiplikGet S1'.sahiplik bb = some (Sahip.thread
                  ({ ctx' with ifade := Ifade.esles s' n d y } : ThreadCtx).tid) := by
              intro hz3
              obtain ⟨h_pre, h_pres⟩ := h_B z bb h_bb2 h_yz
              have h_pre2 : bolgeOrtamGet S.bolge z = some bb := h_pre
              have h_own0 := h_konf.2.2.2.2.2.2.2.1 ctx h_ctx_in z
                (by rw [h_if]; exact hz3) bb h_pre2 h_yz
              have h_tid2 : ({ ctx' with ifade := Ifade.esles s' n d y }
                  : ThreadCtx).tid = ctx.tid := h_tid
              rw [h_tid2]
              exact h_pres h_own0
            cases hz2 with
            | esles_skrut _ _ _ _ _ h_s' =>
                exact h_konf1'.2.2.2.2.2.2.2.1 ctx' h_ctx'_in z
                  (by rw [h_if']; exact h_s') bb h_bb2 h_yz
            | esles_eslesen _ _ _ _ _ h_d' =>
                exact h_dal (HedefVar.esles_eslesen s n d y z h_d')
            | esles_kalan _ _ _ _ _ h_y' =>
                exact h_dal (HedefVar.esles_kalan s n d y z h_y')
          · exact h_konf1'.2.2.2.2.2.2.2.1 c
              (by rw [h_t1']
                  exact List.mem_append.mpr (Or.inr (List.Mem.tail _ h3)))
              z hz bb h_bb h_yz
      · intro c h_mem bb hb h_kayit h_yz
        ...
        · rcases List.mem_cons.mp h2 with he | h3
          · subst he
            have hb2 : HedefBolge (Ifade.esles s' n d y) bb := hb
            have h_ctx'_in : ctx' ∈ S1'.thread := by
              rw [h_t1']; exact List.mem_append.mpr (Or.inr (List.Mem.head _))
            have h_dal : ∀ (hb3 : HedefBolge (Ifade.esles s n d y) bb),
                sahiplikGet S1'.sahiplik bb = some (Sahip.thread
                  ({ ctx' with ifade := Ifade.esles s' n d y } : ThreadCtx).tid) := by
              intro hb3
              obtain ⟨x0, h_x0⟩ := h_kayit
            ...
            | esles_skrut _ _ _ _ _ h_s' =>
                exact h_konf1'.2.2.2.2.2.2.2.2.1 ctx' h_ctx'_in bb
                  (by rw [h_if']; exact h_s') h_kayit h_yz
            | esles_eslesen _ _ _ _ _ h_d' =>
                exact h_dal (HedefBolge.esles_eslesen s n d y bb h_d')
            | esles_kalan _ _ _ _ _ h_y' =>
                exact h_dal (HedefBolge.esles_kalan s n d y bb h_y')
          · exact h_konf1'.2.2.2.2.2.2.2.2.1 c
              (by rw [h_t1']
                  exact List.mem_append.mpr (Or.inr (List.Mem.tail _ h3)))
              bb hb h_kayit h_yz
      · have h13 := h_konf1'.2.2.2.2.2.2.2.2.2.2.2.2.1
        rw [h_t1'] at h13
        exact tidAyrik_degisim h13 _ rfl
  -- D-334: topla cong'lari — sEgerCong ile ayni iskelet.
  -- D-338: bolme cong'lari (asagidaki topla bloklarinin aynasi).
  | sBolCongSol S S' S1 S1' ts1 ts2 ts2' ctx ctx' a a' b h_t h_if h_S1 h_inner h_t1' h_tid h_if' h_yan h_S' ih =>
      intro h_konf
      subst h_S1
      have h_ctx_in : ctx ∈ S.thread := by
      ...
      · intro c h_mem z hz bb h_bb h_yz
        ...
        · rcases List.mem_cons.mp h2 with he | h3
          · subst he
            have hz2 : HedefVar (Ifade.bol a' b) z := hz
            have h_bb2 : bolgeOrtamGet S1'.bolge z = some bb := h_bb
            have h_ctx'_in : ctx' ∈ S1'.thread := by
              rw [h_t1']; exact List.mem_append.mpr (Or.inr (List.Mem.head _))
            cases hz2 with
            ...
            | bol_sag _ _ _ h_b' =>
                ...
                have h_pre2 : bolgeOrtamGet S.bolge z = some bb := h_pre
                have h_own0 := h_konf.2.2.2.2.2.2.2.1 ctx h_ctx_in z
                  (by rw [h_if]; exact HedefVar.bol_sag a b z h_b') bb h_pre2 h_yz
                have h_tid2 : ({ ctx' with ifade := Ifade.bol a' b }
                    : ThreadCtx).tid = ctx.tid := h_tid
                show sahiplikGet S1'.sahiplik bb = some (Sahip.thread
                  ({ ctx' with ifade := Ifade.bol a' b } : ThreadCtx).tid)
                rw [h_tid2]
                exact h_pres h_own0
          · exact h_konf1'.2.2.2.2.2.2.2.1 c
              (by rw [h_t1']
      ...
      · intro c h_mem bb hb h_kayit h_yz
        ...
        · rcases List.mem_cons.mp h2 with he | h3
          · subst he
            have hb2 : HedefBolge (Ifade.bol a' b) bb := hb
            have h_ctx'_in : ctx' ∈ S1'.thread := by
              rw [h_t1']; exact List.mem_append.mpr (Or.inr (List.Mem.head _))
            cases hb2 with
            | bol_sol _ _ _ h_a' =>
            ...
            | bol_sag _ _ _ h_b' =>
                ...
                have h_prex2 : bolgeOrtamGet S.bolge x0 = some bb := h_prex
                have h_own0 := h_konf.2.2.2.2.2.2.2.2.1 ctx h_ctx_in bb
                  (by rw [h_if]; exact HedefBolge.bol_sag a b bb h_b')
                  ⟨x0, h_prex2⟩ h_yz
                have h_tid2 : ({ ctx' with ifade := Ifade.bol a' b }
                    : ThreadCtx).tid = ctx.tid := h_tid
                show sahiplikGet S1'.sahiplik bb = some (Sahip.thread
  ...
  | sKalanCongSol S S' S1 S1' ts1 ts2 ts2' ctx ctx' a a' b h_t h_if h_S1 h_inner h_t1' h_tid h_if' h_yan h_S' ih =>
      ...
      · intro c h_mem z hz bb h_bb h_yz
        ...
        · rcases List.mem_cons.mp h2 with he | h3
          · subst he
            have hz2 : HedefVar (Ifade.kalan a' b) z := hz
            have h_bb2 : bolgeOrtamGet S1'.bolge z = some bb := h_bb
            have h_ctx'_in : ctx' ∈ S1'.thread := by
              rw [h_t1']; exact List.mem_append.mpr (Or.inr (List.Mem.head _))
            cases hz2 with
            ...
            | kalan_sag _ _ _ h_b' =>
                ...
                have h_pre2 : bolgeOrtamGet S.bolge z = some bb := h_pre
                have h_own0 := h_konf.2.2.2.2.2.2.2.1 ctx h_ctx_in z
                  (by rw [h_if]; exact HedefVar.kalan_sag a b z h_b') bb h_pre2 h_yz
                have h_tid2 : ({ ctx' with ifade := Ifade.kalan a' b }
                    : ThreadCtx).tid = ctx.tid := h_tid
                show sahiplikGet S1'.sahiplik bb = some (Sahip.thread
                  ({ ctx' with ifade := Ifade.kalan a' b } : ThreadCtx).tid)
                rw [h_tid2]
                exact h_pres h_own0
          · exact h_konf1'.2.2.2.2.2.2.2.1 c
              (by rw [h_t1']
      ...
      · intro c h_mem bb hb h_kayit h_yz
        ...
        · rcases List.mem_cons.mp h2 with he | h3
          · subst he
            have hb2 : HedefBolge (Ifade.kalan a' b) bb := hb
            have h_ctx'_in : ctx' ∈ S1'.thread := by
              rw [h_t1']; exact List.mem_append.mpr (Or.inr (List.Mem.head _))
            cases hb2 with
            | kalan_sol _ _ _ h_a' =>
            ...
            | kalan_sag _ _ _ h_b' =>
                ...
                have h_prex2 : bolgeOrtamGet S.bolge x0 = some bb := h_prex
                have h_own0 := h_konf.2.2.2.2.2.2.2.2.1 ctx h_ctx_in bb
                  (by rw [h_if]; exact HedefBolge.kalan_sag a b bb h_b')
                  ⟨x0, h_prex2⟩ h_yz
                have h_tid2 : ({ ctx' with ifade := Ifade.kalan a' b }
                    : ThreadCtx).tid = ctx.tid := h_tid
                show sahiplikGet S1'.sahiplik bb = some (Sahip.thread
  ...
  | sBolCongSag S S' S1 S1' ts1 ts2 ts2' ctx ctx' v b b' h_t h_if h_S1 h_inner h_t1' h_tid h_if' h_yan h_S' ih =>
      ...
      obtain ⟨h_nei, h_A, h_B⟩ :=
        adim_odak_yuku Γ Δ Ρ _ S1' h_inner h_konf1
          ts1 ts2 { ctx with ifade := b } ts2' ctx' rfl h_t1' h_yan
      subst h_S'
      have h_beq : S.bolge = Ρ := h_konf.2.2.2.2.2.1
      have h_beq1 : S1'.bolge = Ρ1' := h_konf1'.2.2.2.2.2.1
      refine ⟨Ρ1', h_konf1'.1, ?_, h_konf1'.2.2.1, h_konf1'.2.2.2.1,
        ...
        · rcases List.mem_cons.mp h2 with he | h3
          · subst he
            ...
            | HasType.t_bol _ _ _ _ hta htb,
              ...
              rw [← h_beq] at hrb
              obtain ⟨Λb'', Ρb'', h_tyb', h_kb, h_ib⟩ :=
                h_A Tip.scalar Λ0 Ρ0 ⟨htb, hlb, hrb⟩
              rw [h_if'] at h_tyb'
              rw [h_beq1] at h_tyb'
              exact ⟨Tip.scalar, Λb'', Ρb'',
                ⟨HasType.t_bol _ _ _ _ hta h_tyb'.hasType,
      ...
      · intro c h_mem z hz bb h_bb h_yz
        ...
        · rcases List.mem_cons.mp h2 with he | h3
          · subst he
            have hz2 : HedefVar (Ifade.bol (.sabit v) b') z := hz
            have h_bb2 : bolgeOrtamGet S1'.bolge z = some bb := h_bb
            have h_ctx'_in : ctx' ∈ S1'.thread := by
              rw [h_t1']; exact List.mem_append.mpr (Or.inr (List.Mem.head _))
            cases hz2 with
            | bol_sol _ _ _ h_sv => nomatch h_sv
            | bol_sag _ _ _ h_b'' =>
                exact h_konf1'.2.2.2.2.2.2.2.1 ctx' h_ctx'_in z
                  (by rw [h_if']; exact h_b'') bb h_bb2 h_yz
          · exact h_konf1'.2.2.2.2.2.2.2.1 c
              (by rw [h_t1']
                  exact List.mem_append.mpr (Or.inr (List.Mem.tail _ h3)))
              z hz bb h_bb h_yz
      · intro c h_mem bb hb h_kayit h_yz
        ...
        · rcases List.mem_cons.mp h2 with he | h3
          · subst he
            have hb2 : HedefBolge (Ifade.bol (.sabit v) b') bb := hb
            have h_ctx'_in : ctx' ∈ S1'.thread := by
              rw [h_t1']; exact List.mem_append.mpr (Or.inr (List.Mem.head _))
            cases hb2 with
            | bol_sol _ _ _ h_sv => nomatch h_sv
            | bol_sag _ _ _ h_b'' =>
                exact h_konf1'.2.2.2.2.2.2.2.2.1 ctx' h_ctx'_in bb
                  (by rw [h_if']; exact h_b'') h_kayit h_yz
          · exact h_konf1'.2.2.2.2.2.2.2.2.1 c
              (by rw [h_t1']
                  exact List.mem_append.mpr (Or.inr (List.Mem.tail _ h3)))
              bb hb h_kayit h_yz
  ...
  | sKalanCongSag S S' S1 S1' ts1 ts2 ts2' ctx ctx' v b b' h_t h_if h_S1 h_inner h_t1' h_tid h_if' h_yan h_S' ih =>
      ...
      · intro c h_mem z hz bb h_bb h_yz
        ...
        · rcases List.mem_cons.mp h2 with he | h3
          · subst he
            have hz2 : HedefVar (Ifade.kalan (.sabit v) b') z := hz
            have h_bb2 : bolgeOrtamGet S1'.bolge z = some bb := h_bb
            have h_ctx'_in : ctx' ∈ S1'.thread := by
              rw [h_t1']; exact List.mem_append.mpr (Or.inr (List.Mem.head _))
            cases hz2 with
            | kalan_sol _ _ _ h_sv => nomatch h_sv
            | kalan_sag _ _ _ h_b'' =>
                exact h_konf1'.2.2.2.2.2.2.2.1 ctx' h_ctx'_in z
                  (by rw [h_if']; exact h_b'') bb h_bb2 h_yz
          · exact h_konf1'.2.2.2.2.2.2.2.1 c
      ...
      · intro c h_mem bb hb h_kayit h_yz
        ...
        · rcases List.mem_cons.mp h2 with he | h3
          · subst he
            have hb2 : HedefBolge (Ifade.kalan (.sabit v) b') bb := hb
            have h_ctx'_in : ctx' ∈ S1'.thread := by
              rw [h_t1']; exact List.mem_append.mpr (Or.inr (List.Mem.head _))
            cases hb2 with
            | kalan_sol _ _ _ h_sv => nomatch h_sv
  ...
  | sToplaCongSol S S' S1 S1' ts1 ts2 ts2' ctx ctx' a a' b h_t h_if h_S1 h_inner h_t1' h_tid h_if' h_yan h_S' ih =>
      ...
      · intro c h_mem z hz bb h_bb h_yz
        ...
        · rcases List.mem_cons.mp h2 with he | h3
          · subst he
            have hz2 : HedefVar (Ifade.topla a' b) z := hz
            have h_bb2 : bolgeOrtamGet S1'.bolge z = some bb := h_bb
            have h_ctx'_in : ctx' ∈ S1'.thread := by
              rw [h_t1']; exact List.mem_append.mpr (Or.inr (List.Mem.head _))
            cases hz2 with
            ...
            | topla_sag _ _ _ h_b' =>
                ...
                have h_pre2 : bolgeOrtamGet S.bolge z = some bb := h_pre
                have h_own0 := h_konf.2.2.2.2.2.2.2.1 ctx h_ctx_in z
                  (by rw [h_if]; exact HedefVar.topla_sag a b z h_b') bb h_pre2 h_yz
                have h_tid2 : ({ ctx' with ifade := Ifade.topla a' b }
                    : ThreadCtx).tid = ctx.tid := h_tid
                show sahiplikGet S1'.sahiplik bb = some (Sahip.thread
                  ({ ctx' with ifade := Ifade.topla a' b } : ThreadCtx).tid)
                rw [h_tid2]
                exact h_pres h_own0
          · exact h_konf1'.2.2.2.2.2.2.2.1 c
              (by rw [h_t1']
      ...
      · intro c h_mem bb hb h_kayit h_yz
        ...
        · rcases List.mem_cons.mp h2 with he | h3
          · subst he
            have hb2 : HedefBolge (Ifade.topla a' b) bb := hb
            have h_ctx'_in : ctx' ∈ S1'.thread := by
              rw [h_t1']; exact List.mem_append.mpr (Or.inr (List.Mem.head _))
            cases hb2 with
            | topla_sol _ _ _ h_a' =>
            ...
            | topla_sag _ _ _ h_b' =>
                ...
                have h_prex2 : bolgeOrtamGet S.bolge x0 = some bb := h_prex
                have h_own0 := h_konf.2.2.2.2.2.2.2.2.1 ctx h_ctx_in bb
                  (by rw [h_if]; exact HedefBolge.topla_sag a b bb h_b')
                  ⟨x0, h_prex2⟩ h_yz
                have h_tid2 : ({ ctx' with ifade := Ifade.topla a' b }
                    : ThreadCtx).tid = ctx.tid := h_tid
                show sahiplikGet S1'.sahiplik bb = some (Sahip.thread
  ...
  | sCarpCongSol S S' S1 S1' ts1 ts2 ts2' ctx ctx' a a' b h_t h_if h_S1 h_inner h_t1' h_tid h_if' h_yan h_S' ih =>
      ...
      · intro c h_mem z hz bb h_bb h_yz
        ...
        · rcases List.mem_cons.mp h2 with he | h3
          · subst he
            have hz2 : HedefVar (Ifade.carp a' b) z := hz
            have h_bb2 : bolgeOrtamGet S1'.bolge z = some bb := h_bb
            have h_ctx'_in : ctx' ∈ S1'.thread := by
              rw [h_t1']; exact List.mem_append.mpr (Or.inr (List.Mem.head _))
            cases hz2 with
            ...
            | carp_sag _ _ _ h_b' =>
                ...
                have h_pre2 : bolgeOrtamGet S.bolge z = some bb := h_pre
                have h_own0 := h_konf.2.2.2.2.2.2.2.1 ctx h_ctx_in z
                  (by rw [h_if]; exact HedefVar.carp_sag a b z h_b') bb h_pre2 h_yz
                have h_tid2 : ({ ctx' with ifade := Ifade.carp a' b }
                    : ThreadCtx).tid = ctx.tid := h_tid
                show sahiplikGet S1'.sahiplik bb = some (Sahip.thread
                  ({ ctx' with ifade := Ifade.carp a' b } : ThreadCtx).tid)
                rw [h_tid2]
                exact h_pres h_own0
          · exact h_konf1'.2.2.2.2.2.2.2.1 c
              (by rw [h_t1']
      ...
      · intro c h_mem bb hb h_kayit h_yz
        ...
        · rcases List.mem_cons.mp h2 with he | h3
          · subst he
            have hb2 : HedefBolge (Ifade.carp a' b) bb := hb
            have h_ctx'_in : ctx' ∈ S1'.thread := by
              rw [h_t1']; exact List.mem_append.mpr (Or.inr (List.Mem.head _))
            cases hb2 with
            | carp_sol _ _ _ h_a' =>
            ...
            | carp_sag _ _ _ h_b' =>
                ...
                have h_prex2 : bolgeOrtamGet S.bolge x0 = some bb := h_prex
                have h_own0 := h_konf.2.2.2.2.2.2.2.2.1 ctx h_ctx_in bb
                  (by rw [h_if]; exact HedefBolge.carp_sag a b bb h_b')
                  ⟨x0, h_prex2⟩ h_yz
                have h_tid2 : ({ ctx' with ifade := Ifade.carp a' b }
                    : ThreadCtx).tid = ctx.tid := h_tid
                show sahiplikGet S1'.sahiplik bb = some (Sahip.thread
  ...
  | sToplaCongSag S S' S1 S1' ts1 ts2 ts2' ctx ctx' v b b' h_t h_if h_S1 h_inner h_t1' h_tid h_if' h_yan h_S' ih =>
      ...
      · intro c h_mem z hz bb h_bb h_yz
        ...
        · rcases List.mem_cons.mp h2 with he | h3
          · subst he
            have hz2 : HedefVar (Ifade.topla (.sabit v) b') z := hz
            have h_bb2 : bolgeOrtamGet S1'.bolge z = some bb := h_bb
            have h_ctx'_in : ctx' ∈ S1'.thread := by
              rw [h_t1']; exact List.mem_append.mpr (Or.inr (List.Mem.head _))
            cases hz2 with
            | topla_sol _ _ _ h_sv => nomatch h_sv
            | topla_sag _ _ _ h_b'' =>
                exact h_konf1'.2.2.2.2.2.2.2.1 ctx' h_ctx'_in z
                  (by rw [h_if']; exact h_b'') bb h_bb2 h_yz
          · exact h_konf1'.2.2.2.2.2.2.2.1 c
      ...
      · intro c h_mem bb hb h_kayit h_yz
        ...
        · rcases List.mem_cons.mp h2 with he | h3
          · subst he
            have hb2 : HedefBolge (Ifade.topla (.sabit v) b') bb := hb
            have h_ctx'_in : ctx' ∈ S1'.thread := by
              rw [h_t1']; exact List.mem_append.mpr (Or.inr (List.Mem.head _))
            cases hb2 with
            | topla_sol _ _ _ h_sv => nomatch h_sv
  ...
  | sCarpCongSag S S' S1 S1' ts1 ts2 ts2' ctx ctx' v b b' h_t h_if h_S1 h_inner h_t1' h_tid h_if' h_yan h_S' ih =>
      ...
      · intro c h_mem z hz bb h_bb h_yz
        ...
        · rcases List.mem_cons.mp h2 with he | h3
          · subst he
            have hz2 : HedefVar (Ifade.carp (.sabit v) b') z := hz
            have h_bb2 : bolgeOrtamGet S1'.bolge z = some bb := h_bb
            have h_ctx'_in : ctx' ∈ S1'.thread := by
              rw [h_t1']; exact List.mem_append.mpr (Or.inr (List.Mem.head _))
            cases hz2 with
            | carp_sol _ _ _ h_sv => nomatch h_sv
            | carp_sag _ _ _ h_b'' =>
                exact h_konf1'.2.2.2.2.2.2.2.1 ctx' h_ctx'_in z
                  (by rw [h_if']; exact h_b'') bb h_bb2 h_yz
          · exact h_konf1'.2.2.2.2.2.2.2.1 c
      ...
      · intro c h_mem bb hb h_kayit h_yz
        ...
        · rcases List.mem_cons.mp h2 with he | h3
          · subst he
            have hb2 : HedefBolge (Ifade.carp (.sabit v) b') bb := hb
            have h_ctx'_in : ctx' ∈ S1'.thread := by
              rw [h_t1']; exact List.mem_append.mpr (Or.inr (List.Mem.head _))
            cases hb2 with
            | carp_sol _ _ _ h_sv => nomatch h_sv
  ...
  | sAtamaCong S S' S1 S1' ts1 ts2 ts2' ctx ctx' x e e' h_t h_if h_S1 h_inner h_t1' h_tid h_if' h_yan h_S' ih =>
      ...
      · -- comp 8
        ...
        · rcases List.mem_cons.mp h2 with he | h3
          · subst he
            have hy2 : HedefVar (Ifade.atama x e') y := hy
            have h_bb2 : bolgeOrtamGet S1'.bolge y = some bb := h_bb
            rcases hedefVar_atama_inv hy2 with h_yx | h_e'
            · subst h_yx
              obtain ⟨h_pre, h_pres⟩ := h_B y bb h_bb2 h_yz
              have h_pre2 : bolgeOrtamGet S.bolge y = some bb := h_pre
              have h_own0 := h_konf.2.2.2.2.2.2.2.1 ctx h_ctx_in y
                (by rw [h_if]; exact HedefVar.atama_bas y e) bb h_pre2 h_yz
              have h_own1 := h_pres h_own0
              have h_tid2 : ({ ctx' with ifade := Ifade.atama y e' }
                  : ThreadCtx).tid = ctx.tid := h_tid
              show sahiplikGet S1'.sahiplik bb = some (Sahip.thread
                ({ ctx' with ifade := Ifade.atama y e' } : ThreadCtx).tid)
              rw [h_tid2]
              exact h_own1
            · have h_ctx'_in : ctx' ∈ S1'.thread := by
                rw [h_t1']
      ...
      · -- comp 9
        ...
        · rcases List.mem_cons.mp h2 with he | h3
          · subst he
            have hb2 : HedefBolge (Ifade.atama x e') bb := hb
            have h_e' := hedefBolge_atama_inv hb2
            have h_ctx'_in : ctx' ∈ S1'.thread := by
              rw [h_t1']
              exact List.mem_append.mpr (Or.inr (List.Mem.head _))
            exact h_konf1'.2.2.2.2.2.2.2.2.1 ctx' h_ctx'_in bb
  ...
  | sGuvensizCong S S' S1 S1' ts1 ts2 ts2' ctx ctx' e e' h_t h_if h_S1 h_inner h_t1' h_tid h_if' h_yan h_S' ih =>
      ...
      · -- comp 8
        ...
        · rcases List.mem_cons.mp h2 with he | h3
          · subst he
            have hy2 : HedefVar (Ifade.guvensiz e') y := hy
            have h_bb2 : bolgeOrtamGet S1'.bolge y = some bb := h_bb
            have h_e' := hedefVar_guvensiz_inv hy2
            have h_ctx'_in : ctx' ∈ S1'.thread := by
              rw [h_t1']
              exact List.mem_append.mpr (Or.inr (List.Mem.head _))
            exact h_konf1'.2.2.2.2.2.2.2.1 ctx' h_ctx'_in y
      ...
      · -- comp 9
        ...
        · rcases List.mem_cons.mp h2 with he | h3
          · subst he
            have hb2 : HedefBolge (Ifade.guvensiz e') bb := hb
            have h_e' := hedefBolge_guvensiz_inv hb2
            have h_ctx'_in : ctx' ∈ S1'.thread := by
              rw [h_t1']
              exact List.mem_append.mpr (Or.inr (List.Mem.head _))
            exact h_konf1'.2.2.2.2.2.2.2.2.1 ctx' h_ctx'_in bb
...
/-- TYPED NO-FAULT CATI (Plan v2 §6.3):
    ...
    adim_korunum 21/21 TAM — sorry 0). -/
