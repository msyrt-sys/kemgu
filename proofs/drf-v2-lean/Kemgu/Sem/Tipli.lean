...
theorem typed_bol_sol {Γ : TipOrtam} {Δ : KanalOrtam} {Λ : LineerOrtam}
    {Ρ : BolgeOrtam} {a b : Ifade} {τ : Tip} {Λ' : LineerOrtam} {Ρ' : BolgeOrtam}
    (h : Typed Γ Δ Λ Ρ (Ifade.bol a b) τ Λ' Ρ') :
    ∃ τa Λa Ρa, Typed Γ Δ Λ Ρ a τa Λa Ρa := by
  obtain ⟨ht, hl, hr⟩ := h
  match ht, hl, hr with
  | HasType.t_bol _ _ _ _ hta _,
    LineerTamam.l_bol _ _ Λa _ _ _ hla _,
    RegionTamam.r_bol _ _ Ρa _ _ _ hra _ =>
    exact ⟨Tip.scalar, Λa, Ρa, ⟨hta, hla, hra⟩⟩

/-- D-338: sol operand DEGER iken sag operand tiplidir (sBolCongSag). -/
theorem typed_bol_sag {Γ : TipOrtam} {Δ : KanalOrtam} {Λ : LineerOrtam}
    {Ρ : BolgeOrtam} {v : Deger} {b : Ifade} {τ : Tip}
    {Λ' : LineerOrtam} {Ρ' : BolgeOrtam}
    (h : Typed Γ Δ Λ Ρ (Ifade.bol (Ifade.sabit v) b) τ Λ' Ρ') :
    Typed Γ Δ Λ Ρ b Tip.scalar Λ' Ρ' := by
  obtain ⟨ht, hl, hr⟩ := h
  match ht, hl, hr with
  | HasType.t_bol _ _ _ _ _ htb,
...
/-- D-338: iki operand da deger → bolum tiplidir (sBolTamam sonrasi). -/
theorem typed_bol_atla {Γ : TipOrtam} {Δ : KanalOrtam} {Λ : LineerOrtam}
    {Ρ : BolgeOrtam} {n1 n2 : Int} {τ : Tip} {Λ' : LineerOrtam} {Ρ' : BolgeOrtam}
    (h : Typed Γ Δ Λ Ρ
          (Ifade.bol (Ifade.sabit (.skaler n1)) (Ifade.sabit (.skaler n2)))
          τ Λ' Ρ') :
    Typed Γ Δ Λ Ρ (Ifade.sabit (.skaler (n1 / n2))) τ Λ' Ρ' := by
  obtain ⟨ht, hl, hr⟩ := h
  match ht, hl, hr with
  | HasType.t_bol _ _ _ _ _ _,
    LineerTamam.l_bol _ _ _ _ _ _ hla hlb,
    RegionTamam.r_bol _ _ _ _ _ _ hra hrb =>
    cases hla; cases hlb; cases hra; cases hrb
    exact ⟨HasType.t_sabit _ _ _ _ (DegerTipli.dt_skaler _),
