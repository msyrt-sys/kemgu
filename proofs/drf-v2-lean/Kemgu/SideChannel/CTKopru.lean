...
theorem krun_bol_sol {V : List CT.Ad} (b : Ifade) {d d' : KDurum}
    (h : KRun V d d') :
    KRun V { d with ifade := .bol d.ifade b }
           { d' with ifade := .bol d'.ifade b } := by
  induction h with
  | refl _ => exact KRun.refl _
  | adim dA dB _ hs _ ih =>
      refine KRun.adim _ { dB with ifade := .bol dB.ifade b } _ ?_ ih
      exact Step.sBolCongSol (K V { dA with ifade := .bol dA.ifade b })
...
theorem krun_bol_sag {V : List CT.Ad} (v : Deger) {d d' : KDurum}
    (h : KRun V d d') :
    KRun V { d with ifade := .bol (.sabit v) d.ifade }
...
theorem krun_indeks {V : List CT.Ad} (x : VarId) {d d' : KDurum}
    (h : KRun V d d') :
    KRun V { d with ifade := .indeks x d.ifade }
           { d' with ifade := .indeks x d'.ifade } := by
  induction h with
  | refl _ => exact KRun.refl _
  | adim dA dB _ hs _ ih =>
      refine KRun.adim _ { dB with ifade := .indeks x dB.ifade } _ ?_ ih
      exact Step.sIndeksCong (K V { dA with ifade := .indeks x dA.ifade })
        (K V { dB with ifade := .indeks x dB.ifade }) (K V dA) (K V dB)
...
theorem adim_indeks (V : List CT.Ad) (sigma : Store) (x : CT.Ad) (i : Int)
    (iz : Iz) (z : Zaman) (hx : x ∈ V) :
    Step (K V ⟨sigma, .indeks x (.sabit (.skaler i)), iz, z⟩)
         (K V ⟨sigma, .sabit (.skaler (hucreOku sigma ⟨bol x, i.toNat⟩)),
               .memOku t0 ⟨bol x, i.toNat⟩
...
/-- D-338: bolme taban adimi — `bolOl` olayi uretir (adim_topla'dan
    tek farki budur). -/
theorem adim_bol (V : List CT.Ad) (sigma : Store) (n1 n2 : Int)
    (iz : Iz) (z : Zaman) :
    Step (K V ⟨sigma, .bol (.sabit (.skaler n1)) (.sabit (.skaler n2)), iz, z⟩)
         (K V ⟨sigma, .sabit (.skaler (n1 / n2)),
               .bolOl t0 n1 n2 :: iz, z + 1⟩) :=
  Step.sBolTamam _ _ [] []
...
theorem gomme_sim {V : List CT.Ad} :
  ...
  | c_degisken s x =>
      ...
      refine ⟨.skaler (s x 0), sigma,
        .memOku t0 ⟨bol x, 0⟩ (.skaler (s x 0)) :: iz, z + 1, ?_, hu, ?_,
        fun _ => rfl⟩
      · have hst := adim_indeks V sigma x 0 iz z hx
        rw [hval] at hst
        exact KRun.adim _ _ _ hst (KRun.refl _)
      · show gozlem (Olay.memOku t0 ⟨bol x, 0⟩ (.skaler (s x 0))) :: izGozlem iz = _
        rfl
  ...
  | c_indeks s si x idx ti vi _ ihI =>
      ...
      | g_indeks _ _ h_gi h_si =>
        ...
        have hst := adim_indeks V sgI x vi izI zI hx
        rw [hval] at hst
        refine ⟨.skaler (si x vi.toNat), sgI,
          .memOku t0 ⟨bol x, vi.toNat⟩ (.skaler (si x vi.toNat)) :: izI, zI + 1,
          krun_trans (krun_indeks (V := V) x hrI)
            (KRun.adim _ _ _ hst (KRun.refl _)),
          huI, ?_, fun _ => rfl⟩
        show gozlem (Olay.memOku t0 ⟨bol x, vi.toNat⟩ (.skaler (si x vi.toNat)))
               :: izGozlem izI = _
  ...
  -- iz karsiligi `gomGoz (.oBol v1 v2) = .gBol t0 v1 v2` ile kapanir.
  | c_bol s s1 s2 a b t1 t2 v1 v2 _ _ iha ihb =>
      ...
      | g_bol _ _ h_ga h_sa h_gb h_sb =>
        ...
        rw [hs2 h_sb] at hr2
        refine ⟨.skaler (v1 / v2), sg2, .bolOl t0 v1 v2 :: iz2, z2 + 1,
          krun_trans (krun_bol_sol (V := V) (gom b) hr1)
            (krun_trans (krun_bol_sag (V := V) (.skaler v1) hr2)
              (KRun.adim _ ⟨sg2, .sabit (.skaler (v1 / v2)),
                 .bolOl t0 v1 v2 :: iz2, z2 + 1⟩ _
