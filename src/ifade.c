...
Dugum *parse_tip(Parser *p) {
    ...
    if (t.tip == TOK_VEKTOR) {
        parser_ilerle(p);
        parser_bekle(p, TOK_KUCUK, "P352", "vektor<T, N> icin '<' bekleniyor");
        Dugum *eleman = parse_tip(p);
        parser_bekle(p, TOK_VIRGUL, "P353", "',' bekleniyor (vektor<T, N>)");
        ...
        if (lane_tok.tip == TOK_TAMSAYI) {
            ...
            parser_hata(p, lane_tok, "P354",
                "vektor<T, N> ikinci argumani compile-time tamsayi literal olmali",
                NULL);
        }
        parser_buyuk_ayir(p);
        parser_bekle(p, TOK_BUYUK, "P355", "vektor<...> icin '>' bekleniyor");
