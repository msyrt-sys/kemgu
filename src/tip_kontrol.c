            ...
            for (int i = 0; i < d->veri.cagri.sayi; i++) {
                ...
                if (arg_tip->kategori != TIP_HATA &&
                    ...
                    ct003_leak_kontrol(tk, d->veri.cagri.argumanlar[i],
                                        arg_tip, param_tip)) {
                    /* hata raporlandı */
                } else if (!tip_esit(arg_tip, param_tip) &&
...
TipBilgisi *tip_belirle_beklenen(TipKontrol *tk, const Dugum *d,
    ...
    switch (d->tip) {
        ...
        case DUGUM_CAGRI: {
             ...
             * Bu sayede `de\xc4\x9fi\xc5\x9fken k: sabitsure<tam8> =
             * sabitsure_yarat(1)` gibi durumlarda 1 → tam8 çıkarsanır. */
            ...
            /* Aritmetik/karşılaştırma çağrısı sabitsüre dönerse,
             * beklenen iç T context'iyle inner inference: ikili op için
             * şu an gerekli değil çünkü DUGUM_IKILI ayrı case'de işlenir.
             * (sabitsure_yarat fallback yeterli, normal tip_belirle yolu.) */
            break;
        }

        case DUGUM_IKILI: {
            /* Eğer beklenen sabitsüre<X> ise, operandlar X context'inde
             * çıkarsanır. Bu sayede sabitsüre<dtam8> = sabitsüre_yarat(0) & 15
             * gibi durumlarda 0 ve 15 dtam8 olarak çıkarsanır. */
            if (beklenen->kategori == TIP_SABITSURE) {
                ...
                /* DUGUM_IKILI normal handler'ı taklit et — burada genel
                 * davranışı eksik tutmadan kontrol_ikili_sayisal'a benzer
                 * mantık uygulayabiliriz. Ama daha kolay: tip_belirle'yi
                 * çağıralım çünkü operandların tipleri zaten doğru. */
                /* Geri düş: tip_belirle çağır */
            }
            break;
        }
