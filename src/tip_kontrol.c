...
/* === Madde D: Generic call inference helpers === */
...
TipBilgisi *tip_belirle(TipKontrol *tk, const Dugum *d) {
    ...
    switch (d->tip) {
        ...
        case DUGUM_CAGRI: {
            ...
            /* Madde D: Coklu generic param inference + cagri tip cikarsama.
             * Pass 1: param/arg paralel walk -> baglamalari topla.
             * Pass 2: her arg'i tam substituted param ile karsilastir. */
            GenericBaglama *baglamalar = NULL;
            TipBilgisi **arg_tipler = NULL;
            if (d->veri.cagri.sayi > 0) {
                arg_tipler = (TipBilgisi **)arena_ayir(tk->arena,
                    sizeof(TipBilgisi *) *
                    (size_t)d->veri.cagri.sayi);
            }
            for (int i = 0; i < d->veri.cagri.sayi; i++) {
                TipBilgisi *param_tip = hedef_tip->veri.islev.parametreler[i];
                TipBilgisi *bek_subst = tip_subst_baglamalar(tk, param_tip,
                                                              baglamalar);
                TipBilgisi *arg_tip = tip_belirle_beklenen(tk,
                    d->veri.cagri.argumanlar[i], bek_subst);
                arg_tipler[i] = arg_tip;
                tip_unify(tk, param_tip, arg_tip, &baglamalar);
                if (param_tip && param_tip->kategori == TIP_TEKKEZ) {
                    lineer_tuket_eger_baglamaysa(tk,
                        d->veri.cagri.argumanlar[i]);
                }
            }
            /* Pass 2: tam substituted param vs arg karsilastir */
            for (int i = 0; i < d->veri.cagri.sayi; i++) {
                TipBilgisi *param_tip = hedef_tip->veri.islev.parametreler[i];
                TipBilgisi *full = tip_subst_baglamalar(tk, param_tip,
                                                         baglamalar);
                TipBilgisi *arg_tip = arg_tipler[i];
                if (!arg_tip || arg_tip->kategori == TIP_HATA) continue;
                if (!full) continue;
                if (param_tip_generic_iceriyor_mu(full)) {
                    /* Bound bulunmadi — generic abstract, atla */
                    continue;
                }
                if (!tip_esit(arg_tip, full)) {
                    tip_hata(tk, d->veri.cagri.argumanlar[i], "T001",
                        "arguman tipi parametre tipi ile uyumsuz");
                }
            }
            /* Donus tipi: baglamalari uygula (recursive) */
            TipBilgisi *donus = hedef_tip->veri.islev.donus;
            if (donus) {
                TipBilgisi *sb = tip_subst_baglamalar(tk, donus, baglamalar);
                if (sb) return sb;
            }
            return donus;
        }
